...
- Workflow metadata is declarative and frozen: `MODULES` and `AUDIT_UNITS` must be direct `deepFreezeRecords([...])` initializers. Inline consumption of those arrays is supported, but direct RHS-leading reference aliases or destructuring of either root array or its nested arrays are unsupported and rejected because bounded static scope analysis cannot prove complete mutation provenance. Primitive `.length` bindings and derived `.map()`/`.filter()` result bindings are allowed because they do not alias the frozen graph; mutations through that graph remain rejected.
