...
**v0.4.11 — §A1 supply-chain lifecycle gaps (2026-07-09).** Three rule bullets in §A1 plus a documentation/grounding refresh, no new categories (still **58**), no tier change, from a dedicated supply-chain/build gap audit (`docs/reviews/gap-audit-supply-chain-build.md`). All three additions are the *lifecycle* material §A1 lacked — not "which crate name" (already covered) but what happens to a trusted dependency's bytes over time. §A1's lockfile paragraph gains **yanked-version handling** (🟡, →🔴 when the yank is an advisory): `cargo build --locked` against a `Cargo.lock` pinning a since-yanked version warns-not-errors — exit 0, green in CI for months against a version yanked for a security reason — while only a *fresh* resolution refuses it; REQUIRED `cargo deny check advisories` / `cargo audit -D warnings` turns the silent warning red with a stated reason, and the fix is a *targeted* `cargo update -p <crate>`, never the reflexive blanket `cargo update` (Tier-A residue that swaps many other pins). §A1's build-time paragraph gains **network access in your *own* `build.rs`** (🟡) — the **write-direction twin** of the existing read-direction malicious-dependency rule: `curl`/`git clone`/`reqwest::blocking` in your build script pulls *unpinned bytes into the build outside the lockfile*, so `--locked`/`--offline`/`cargo vendor` give it zero integrity, it is MITM-able and non-reproducible; REQUIRED vendor the artifact or fetch it in a separate CI step with a hardcoded SHA-256 + offline fallback (build-time codegen from a *local* schema/toolchain is explicitly fine). §A1's BANNED/REQUIRED lists gain **unpinned `[patch]`/git overrides** (🔴 unpinned-HEAD/unnamed fork, 🟡 rev-pinned user-approved temporary patch): `[patch.crates-io] tokio = { git = "…", branch = "…" }` with no `rev` follows remote HEAD and walks *around* the registry-keyed `cargo audit`/`vet`/RUSTSEC stack — the same unverified-trust decision as adding an unknown dependency name, just disguised as "it's still `tokio`, just my fork"; REQUIRED a full-commit `rev`, a user-approved repo, a post-flight flag, and a stated removal condition. The incident base is refreshed (`evm-units`/`uniswap-utils`, `finch-rust`/`sha-rust`, the 2026-02 crates.io policy update, the TrapDoor campaign) and §A1's proc-macro-trust sentence finally cites the serde_derive precompiled-binary episode (2023, reverted v1.0.184). Grounded in the Cargo Book (`cargo yank`, Build Scripts, Overriding/git Dependencies), cargo#5720, reproducible-builds.org, the cargo-audit/cargo-deny yanked-check docs, the rust-lang malicious-crate advisories, and the `onering` account-compromise (Aikido 2026). **PATCH** bump (bullets/documentation only, no new lettered category — per the v0.4.3/v0.4.6/v0.4.7/v0.4.9/v0.4.10 sibling-bullet precedent). See [`CHANGELOG.md`](CHANGELOG.md).

**v0.4.10 — §B14/§B3a/§B13/§B17 concurrency & admission-control exhaustion gaps (2026-07-09).** Five bullets/amendments, no new categories (still **58**), no tier change, from a dedicated gap audit (`docs/reviews/gap-audit-concurrency-exhaustion.md`). Thesis: both concurrency modules are dense on unbounded *data* growth (§B14) and *leaked* resources (§F3/§B21) but blind to unbounded *admission* — object counts, key cardinality, synchronized load — where each individual object is perfectly managed and the exhaustion lives one level up. §B14 gains **unbounded task/connection admission** (🔴 peer-extendable — an accept loop that spawns one task per connection with no cap grows nothing the existing channel/`Vec`/`FuturesUnordered` bullets name; the *count* of tasks/sockets is the resource, attacker sets N → OOM/FD exhaustion, CWE-770; fix: `Arc<Semaphore>` + `acquire_owned().await` *before* `tokio::spawn` with the permit *moved into* the task — the two LLM-typical wrong fixes, acquiring inside the task or dropping the permit in the loop, are named), **accept-error classification** (🟡 — `?` on `accept()` kills the loop, `warn!;continue` busy-spins on `EMFILE`; classify fatal vs transient-with-backoff, and explicitly *not* §B3a's release-and-return), and **insert-only unbounded caches/registries** (🔴 attacker-extendable keys — a per-user/IP/path `HashMap`/`DashMap` with no eviction grows forever; a cache *has* a consumer so the "`Vec` with no consumer" bullet does not match — the defect is *no eviction*; bound via `lru`/`moka` or argue closed cardinality, CWE-770/400), plus a warning line on §B13's endorsed per-key `Arc<OnceCell>` map (that map itself is insert-only). §B3a's REQUIRED backoff clause is amended in place: back off exponentially **with jitter** — deterministic backoff synchronizes N instances into thundering-herd waves; add a retry budget when the caller fans out ("reconnect on disconnect" added to the trigger row). §B17 gains a third reentrance flavor (RefCell→panic, tokio Mutex→deadlock, **`RwLock` reentrant read→deadlock** — std's priority policy is unspecified/OS-dependent, tokio's is documented write-preferring, so a second `.read()` reached through a call graph while a writer waits deadlocks; passes on dev, hangs on prod OS), with an §E4 cross-ref. Grounded in tokio `Semaphore`/`RwLock`/listener docs, tokio discussions #5091/#2648, std `RwLock` docs + rust-lang/rfcs#2674, `accept(2)`, AWS "Exponential Backoff And Jitter" + Builders' Library, CWE-770/400. The two cache CVEs (CVE-2026-33012 / CVE-2026-41310) are cited honestly as cross-language *pattern* grounding, not Rust incidents. **PATCH** bump (bullets/amendments only, no new lettered category — per the v0.4.3/v0.4.6/v0.4.7/v0.4.9 sibling-bullet precedent). See [`CHANGELOG.md`](CHANGELOG.md).

**v0.4.9 — §B7/§B20 deserialization & parsing DoS gaps (2026-07-09).** Five bullets/sub-clauses, no new categories (still **58**), no tier change, from a dedicated deserialization-DoS gap audit (`docs/reviews/gap-audit-deserialization-dos.md`). §B7 gains a **decompression-bomb** BANNED bullet (🔴 — capping the *compressed* input does not cap the *decompressed* output; `GzDecoder::read_to_end` amplifies ~1 KiB → multi-GiB with no clampable `n`, and the `zip` crate's internal `.take(compressed_size)` is active false safety — bounds the compressed side only; fix: `.take(MAX+1)` on the *decoder*, CWE-409), a recursion sub-clause on the existing depth bullet (serde_json's 128 limit is *parse-phase only* — re-deserializing an already-built `serde_json::Value` via `from_value`/`IgnoredAny`/`flatten`/`untagged` bypasses it → uncatchable stack overflow; serde_yaml's alias/anchor recursion was fixed in 0.8.4 (RUSTSEC-2018-0005) but the crate is now unmaintained — re-verify; mitigate with `serde_stacker` or cap depth pre-parse), and a one-line `checked_mul` cross-ref (a wrapping `count * item_size` in release silently *passes* a size clamp, then the original count OOMs — CWE-190→789). §B20 extends its existing `flatten` bullet with the **buffer-everything cliff** (`flatten`/tagged enums flip serde from streaming into "materialize the whole body into a `Content` tree then re-deserialize" — ~2× slower, allocates the full body, errors lose position; serde #2363 measures ~50–86%; cross-ref §E2) and adds the missing `deny_unknown_fields` direction (REQUIRED on untrusted request structs so unknown/typo'd/mass-assignment fields are *rejected*, not silently ignored — the opposite of the existing self-typo warning; plus JSON duplicate-key last-wins as a parser-differential smuggling vector, RFC 8259 §4). Calibration preserved: `flatten` is not banned (a real feature — only shipping it un-anticipated on a hot/untrusted path is the finding); `deny_unknown_fields` is REQUIRED only where unexpected-field rejection matters, not on a forward-compatible permissive API. **PATCH** bump (bullets/sub-clauses only, no new lettered category — per the v0.4.3/v0.4.6/v0.4.7 sibling-bullet precedent). See [`CHANGELOG.md`](CHANGELOG.md).

**v0.4.8 — §B25a + FFI/unsafe boundary gaps (2026-07-09).** Five bullets across §B5/§B25 plus one new lettered sub-category **§B25a** (the C library's own concurrency contract), from a dedicated FFI/unsafe gap audit; still **58** numbered categories (§B25a is counted under §B25, exactly as §B18a is under §B18). §B25a is a genuinely separate failure locus from §B25 — the soundness property lives in *the C library's* code (global mutable state, non-reentrancy, one-time init), not in the wrapper's own data, and the reflexive §B18 per-handle `Arc<Mutex<_>>` fix *actively masks* it: two independently-locked handles still race on the library's global (CVE-2020-26235 / RUSTSEC-2020-0071/0159, shipped for years in `time`/`chrono`) — cite the library's documented thread-safety level, use a process-global lock for a non-reentrant library, and capture `errno` immediately. §B25 gains callback-context UAF (C fires the callback after the context is freed / a re-invocable trampoline that reclaims an owning `Box<F>` double-drops → borrow, unregister→drain→free), exported entry points trusting the type system (`#[no_mangle] extern "C" fn(&T, &str, …)` — C passes NULL/non-UTF-8/bad-discriminant → UB in "safe" Rust; take `*const/*mut` + primitives and validate first — the read-direction mirror of §B5's validate-before-mint), and per-function ownership audit (take/copy/borrow/return-owned differ across the same C library — cite the C doc line per function; undocumented → block-and-ask). §B5 gains `union` field reads (no active-field tracking — a wrong/uninitialized field read is instant UB; tag-checked `#[repr(C)]` accessor + `// SAFETY:`, or use an `enum`; the word "union" appeared nowhere before). Grounded in the Rust Reference UB list + Unions page, the Nomicon FFI chapter, the Rust 2024 `unsafe(no_mangle)` guide, and RUSTSEC-2020-0071/0159. **PATCH** bump, per the §B18a/§C1a (v0.3.2) and §B24-not-§B24a (v0.4.7) precedent: a new lettered sub-category that leaves the numbered count unchanged is a PATCH-shaped event in this repo, not MINOR. See [`CHANGELOG.md`](CHANGELOG.md).

**v0.4.7 — §B12/§B24 crypto & secrets gaps (2026-07-09).** Six bullets, no new categories (still **58**): a dedicated gap audit filled the highest-frequency crypto mistakes in LLM-generated web code that were absent from the skill ("salt", "certificate", "aud" appeared nowhere before). §B12 gains JWT claim validation (`aud`/`iss`/`required_spec_claims`, not just `alg`), TLS validation bypass (`danger_accept_invalid_certs` / no-op cert verifier → MITM, CWE-295 — pin the CA), KDF salt misuse + below-OWASP-floor Argon2/PBKDF2 params (OWASP cited as a living floor), zeroize defeated by moves/reallocation (`Box`/`SecretBox` at creation), and `cargo audit`/RUSTSEC before adding crypto deps (`rsa` Marvin, RUSTSEC-2023-0071). §B24 widened from `==` timing to secret-dependent side channels generally — a decryption-failure oracle (distinguishable padding vs MAC errors, CWE-208/209) is collapsed to one opaque error, with a §C2↔§B24 carve-out ("error context stops at the crypto boundary"). See [`CHANGELOG.md`](CHANGELOG.md).
