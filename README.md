...
**Unreleased (in preparation, not tagged).** The current tree carries the post-v0.6.0 completeness, correctness, currency, validator-hardening, installer, and release-tooling work summarized in [`CHANGELOG.md`](CHANGELOG.md). Repository tooling requires Node.js 24 or newer, and the next package release will carry that floor; the CI definitions cover the current Node 24 line and the exact `24.0.0` floor, but no current-head CI result is claimed here. The validator fixture suite currently has **460** controls. Of these, **397** spawn child processes (**380** validator-entrypoint and **17** focused lexer/helper children), and **63** run in-process (including direct, rule-text, and crude source oracles). The resource-heavy lexer probes use focused short-lived children with a bounded heap cap, and the installer recovery matrix in `dev/test-installer-recovery.mjs` is a separate helper/CI suite not part of that numbered registry. The shared JavaScript scanner rejects mismatched delimiters within its deterministic budget, preserves private-name token roles, tracks class-body roles across brace-bearing `extends` expressions, and distinguishes keyword-named class fields; controls 415–460 cover function/class division, direct function-heritage ordering, class-field initializer roles, completion-loop, workflow-mutation, lexical-cache, and focused-helper boundaries. The round-39 fixing commits (`f5a655e`, `1363cc8`, and `7042ce8`) implement the direct function-heritage, checked-inventory, cross-operation recovery, cleanup-oracle, replacement-inventory, timeout, and Bash 3.2/PowerShell matrix changes identified by the round-39 review. Round 40's independent review of disposition commit `8b2d576` found no P0/P1 but three P2 and four P3 findings; the round-40 fixing commits (`6b435f7`, `090844b`, and `81b5d60`) address the blank-only inventory, independent recovery/debris-oracle, keyword-field, bounded-cache, and publish-timeout findings. Round 41's fixes (`9a675f8`, `c5b959f`, and `322a034`) add the independent-oracle negative, class-field position tracking, and structured focused-helper controls. The former cumulative lexer retention is now bounded, and large-input probes are isolated from the long-lived fixture parent; focused lexer children report child-owned heap/RSS telemetry only in terminal JSON, while a fatal or killed child has no terminal sample. Parent progress attribution remains diagnostic evidence rather than a failing-child memory proof. Release gates are a clean ordinary validator run, the `installer-boundaries` CI job's generated `--list` loops over every same/cross Node case (**578/578 cases** across `node-claude` and `node-codex`), independent HS review, and current-head CI; validator memory evidence is reported separately by the focused-child protocol. The documented Windows contract remains process interruption, not sudden-power-loss durability. The next release is planned as **MINOR `0.7.0`**; manifests and the release banner remain `v0.6.0` until that release is cut. Release readiness requires passing clean validator runs, the complete recovery matrix, independent HS review of the fixing head, current-head CI on the reviewed release candidate, and resolution of any remaining findings.
...
rollback and recovery checks, `validate-lexer-probes.mjs`, the focused child-process probes used by
resource-heavy lexer controls, and `test-installer-recovery.mjs`, the separate generated boundary
matrix for installer interruption/restart behavior. Set `RUST_INTEL_POWERSHELL_EXECUTABLE` to
select the PowerShell runtime used by the helper (`pwsh` by default; `powershell.exe` exercises
the Windows PowerShell 5.1 surface used by the `.bat` wrappers). Its current-Bash and Node results include
...
│   ├── validate-lexer-observations.mjs  # Shared focused-probe semantic oracle
