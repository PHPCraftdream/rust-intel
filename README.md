...
**Unreleased (in preparation, not tagged).** The current tree carries the post-v0.6.0 completeness, correctness, currency, validator-hardening, installer, and release-tooling work summarized in [`CHANGELOG.md`](CHANGELOG.md). Repository tooling requires Node.js 24 or newer, and the next package release will carry that floor; CI covers the current Node 24 line and the exact `24.0.0` floor. The validator fixture suite currently has **422** controls. Of those, **372** spawn validator children and **50** run in-process; the installer recovery matrix in `dev/test-installer-recovery.mjs` is a separate helper/CI suite and is not part of that numbered total. The shared JavaScript scanner rejects mismatched delimiters within its deterministic budget, preserves private-name token roles, and now distinguishes function/class-expression division from declaration regexps; controls 415–422 cover those expression and workflow-mutation boundaries. Installer transaction recovery now records restartable restore states, and the helper matrix covers rollback, restore, fresh, upgrade, sparse, install, and uninstall boundaries across the supported surfaces; the documented Windows contract remains process interruption, not sudden-power-loss durability. The next release is planned as **MINOR `0.7.0`**; manifests and the release banner remain `v0.6.0` until that release is cut. Release readiness remains gated on a fresh independent review and the remaining release checks.
