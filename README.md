...
**Unreleased (in preparation, not tagged).** The current tree carries the post-v0.6.0 completeness, correctness, currency, validator-hardening, installer, and release-tooling work summarized in [`CHANGELOG.md`](CHANGELOG.md). Repository tooling requires Node.js 24 or newer, and the next package release will carry that floor; the CI definitions cover the current Node 24 line and the exact `24.0.0` floor, but no current-head CI result is claimed here. The validator fixture suite currently has **440** controls. Of those, **374** spawn validator children and **66** run in-process; the installer recovery matrix in `dev/test-installer-recovery.mjs` is a separate helper/CI suite and is not part of that numbered total. The shared JavaScript scanner rejects mismatched delimiters within its deterministic budget, preserves private-name token roles, and tracks class-body roles across brace-bearing `extends` expressions; controls 415–440 cover function/class division, direct function-heritage ordering, completion-loop, and workflow-mutation boundaries. The round-39 fixing commits (`f5a655e`, `1363cc8`, and `7042ce8`) implement the direct function-heritage, checked-inventory, cross-operation recovery, cleanup-oracle, replacement-inventory, timeout, and Bash 3.2/PowerShell matrix changes identified by the round-39 review. Round 40's independent review of disposition commit `8b2d576` found no P0/P1, but retained P2/P3 gaps in blank-only inventory rejection, keyword-named class fields, full-validator OOM diagnosis, and independent cross-operation/debris oracles; this fixing pass also bounds the tag-triggered publish job to 45 minutes. Those fixes and the remaining round-40 work have not yet received an independent HS follow-up, so they are not release closure claims. The documented Windows contract remains process interruption, not sudden-power-loss durability. The next release is planned as **MINOR `0.7.0`**; manifests and the release banner remain `v0.6.0` until that release is cut. Release readiness requires closing all P2/P3 findings, a fresh independent HS review of the fixing head, current-head CI on the reviewed release candidate, and resolution of any remaining findings.
