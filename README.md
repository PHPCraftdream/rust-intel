...
rollback and recovery checks, and `test-installer-recovery.mjs`, the separate generated boundary
matrix for installer interruption/restart behavior. Its current-Bash and Node results include
same- and cross-operation execution evidence, but do not by themselves establish independent
cross-operation postconditions, failed-list propagation, timeout coverage, or the full Bash
3.2/PowerShell runtime contract.
