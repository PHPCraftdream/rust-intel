...
**Unreleased (in preparation, not tagged).** The current tree carries the post-v0.6.0 completeness, correctness, currency, validator-hardening, installer, and release-tooling work summarized in [`CHANGELOG.md`](CHANGELOG.md). Repository tooling requires Node.js 24 or newer, and the next package release will carry that floor; the CI definitions cover the current Node 24 line and the exact `24.0.0` floor, but no current-head CI result is claimed here. The validator fixture suite currently has **422** controls. Of those, **372** spawn validator children and **50** run in-process; the installer recovery matrix in `dev/test-installer-recovery.mjs` is a separate helper/CI suite and is not part of that numbered total. The shared JavaScript scanner rejects mismatched delimiters within its deterministic budget and preserves private-name token roles. The simple function/class-expression division cases are covered, but round 38 found that a brace inside a class `extends` expression can still be misclassified as a regexp and hide a completion or workflow mutation. Installer transaction recovery records restartable restore states, but the round-38 review found incomplete rollback-depth reachability, restore-hook index defects, non-genuine fresh fixtures, inventory drift, and an oracle that does not inspect sibling transaction directories. The documented Windows contract remains process interruption, not sudden-power-loss durability. The next release is planned as **MINOR `0.7.0`**; manifests and the release banner remain `v0.6.0` until that release is cut. Release readiness remains blocked by the open round-38 P1/P2/P3 findings and requires a fresh independent review after their fixes.
