...
**Unreleased (in preparation, not tagged).** The current tree carries the post-v0.6.0 completeness, correctness, currency, validator-hardening, installer, and release-tooling work summarized in [`CHANGELOG.md`](CHANGELOG.md). Repository tooling requires Node.js 24 or newer, and the next package release will carry that floor; the CI definitions cover the current Node 24 line and the exact `24.0.0` floor, but no current-head CI result is claimed here. The validator fixture suite currently has **449** controls. Of those, **376** spawn validator children and **73** run in-process; the installer recovery matrix in `dev/test-installer-recovery.mjs` is a separate helper/CI suite and is not part of that numbered total. The shared JavaScript scanner rejects mismatched delimiters within its deterministic budget, preserves private-name token roles, tracks class-body roles across brace-bearing `extends` expressions, and now distinguishes keyword-named class fields; controls 415–449 cover function/class division, direct function-heritage ordering, completion-loop, workflow-mutation, and lexical-cache boundaries. The round-39 fixing commits (`f5a655e`, `1363cc8`, and `7042ce8`) implement the direct function-heritage, checked-inventory, cross-operation recovery, cleanup-oracle, replacement-inventory, timeout, and Bash 3.2/PowerShell matrix changes identified by the round-39 review. Round 40's independent review of disposition commit `8b2d576` found no P0/P1 but three P2 and four P3 findings; the round-40 fixing commits (`6b435f7`, `090844b`, and `81b5d60`) address the blank-only inventory, independent recovery/debris-oracle, keyword-field, bounded-cache, and publish-timeout findings. The bounded-cache fix reduced retention, but an ordinary fixture run and `npm run validate` still terminate with a V8 heap-allocation failure, so the full validator and generated Node recovery matrix memory behavior require further diagnosis and proof; no release closure or current-head CI result is claimed. The documented Windows contract remains process interruption, not sudden-power-loss durability. The next release is planned as **MINOR `0.7.0`**; manifests and the release banner remain `v0.6.0` until that release is cut. Release readiness requires a passing clean validator run, independent HS review of the fixing head, current-head CI on the reviewed release candidate, and resolution of any remaining findings.
