...
**Unreleased (in preparation, not tagged).** The current tree carries the post-v0.6.0 completeness, correctness, currency, validator-hardening, installer, and release-tooling work summarized in [`CHANGELOG.md`](CHANGELOG.md). Repository tooling requires Node.js 24 or newer, and the next package release will carry that floor; the CI definitions cover the current Node 24 line and the exact `24.0.0` floor, but no current-head CI result is claimed here. The validator fixture suite currently has **430** controls. Of those, **373** spawn validator children and **57** run in-process; the installer recovery matrix in `dev/test-installer-recovery.mjs` is a separate helper/CI suite and is not part of that numbered total. The shared JavaScript scanner rejects mismatched delimiters within its deterministic budget, preserves private-name token roles, and tracks class-body roles across brace-bearing `extends` expressions; controls 415–430 cover function/class division, completion-loop, and workflow-mutation boundaries, but round 39 found an uncovered direct function-heritage shape at the same delimiter depth. The round-38 fixing pass implemented its reported installer and lexer cases, but round 39 found that the broader closure was incomplete: POSIX inventory failures can be masked, cross-operation transaction recovery is not unified for Bash/PowerShell, the Bash-uninstall cleanup oracle and replacement inventory are incomplete, child and job timeouts are absent, and Bash 3.2 runs only a smoke boundary. The Node and current-Bash same-operation matrices passed at the reviewed head; that evidence does not close those cross-operation, generator-failure, timeout, oldest-Bash, or PowerShell-runtime gaps. The documented Windows contract remains process interruption, not sudden-power-loss durability. The next release is planned as **MINOR `0.7.0`**; manifests and the release banner remain `v0.6.0` until that release is cut. Release readiness requires closing all P2/P3 findings in round 39, a fresh independent HS review, and current-head CI on the reviewed release candidate.
...
matrix for installer interruption/restart behavior. Its current-Bash and Node results are useful
same-operation evidence, but do not by themselves establish cross-operation recovery, failed-list
propagation, timeout coverage, or the full Bash 3.2/PowerShell runtime contract.
