...
6. **Show the caller for genuinely multi-lifetime returns.** A function whose returned reference is tied to **more than one** input lifetime (the §B1a laundering shape) requires at least one example call site in a comment or test — a call inserting a short-lived source, then a use of the cache after that source's scope ends — before the signature is final. A plain `&T` derived from a single input does not. See §B1.
...
| `tokio::select! { ... }` with a side effect in an arm's polled future expression (before `=>`) — not the handler after it, which runs only for the winning arm | §B23 (arm side effects) |
...
| `pub enum` / `pub struct` (especially an error enum) in a published library without `#[non_exhaustive]` | §C1a (adding a variant / field is a semver-major break downstream — unless the struct already has a private field, see §C1a) |
...
| `unsafe extern` block containing a `safe fn` item whose signature takes a pointer/handle/length/callback | §B25 (import-direction twin of the exported-entry-point rule — every caller becomes "safe" Rust with no `unsafe` block or `// SAFETY:`; `safe` is valid only for functions correct for ALL inputs, else keep `unsafe fn` + a validating safe wrapper) |
...
| `unsafe { std::env::set_var(...) }` / `unsafe { std::env::remove_var(...) }` anywhere except the top of `main` before any thread or async runtime exists | §B25a + §B5 (process-global C-state race — `cargo fix --edition`'s audit-prompt comment is a prompt to audit, not an audit) |
...
| `.read(...)` / `read_exact` / `next()` on a remote-peer stream with no whole-frame deadline and no idle-deadline mechanism | §F3 (untrusted peer can hold the task forever — a per-read `timeout(dur, read_exact(..))` is not a fix: slowloris-able one-byte-per-window, and a cancelled `read_exact` loses already-consumed bytes, §B3; a fired deadline must be terminal, not retried) |
...
| `bincode`/`postcard`/`rkyv`/`borsh` (de)serialization of a type written to disk/DB/wire, in a diff that inserts or reorders its enum variants or struct fields without a version bump or golden-bytes fixture | §F1 (positional format: declaration order is the persisted spec — the §F4 round-trip stays green while already-stored bytes reinterpret) |
