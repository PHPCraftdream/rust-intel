...
| "extract archive", "unpack tar/zip", "archive destination", "restore an archive" | §B7 decompression/extraction DoS; §C2 path traversal | validate entry paths, reject `..`/absolute paths/symlinks/hardlinks/special files, cap total output and use race-resistant directory APIs when needed |
| "re-deserialize a Value", "from_value", "middleware JSON", "parse YAML config from upload", "already-parsed JSON" | §B7 parser-recursion sub-clause | serde_json's 128 limit is parse-phase only; `from_value`/`IgnoredAny`/`flatten` over an AST bypass it; older/unmaintained serde_yaml versions had uncontrolled recursion — verify the pinned version and cap depth before building an AST |
...
| archive extraction that joins entry names to a destination without rejecting `..`/absolute paths, links, special files, or enforcing a total output quota | §C2 + §B7 (archive traversal/escape or extraction exhaustion) |
| `serde_json::from_value(...)` / `IgnoredAny` on untrusted input; `serde_yaml::from_*` on an untrusted body | §B7 (recursion over an already-built AST bypasses the parse-phase depth limit → uncatchable stack overflow; verify the pinned serde_yaml version) |
...
| `union` keyword / `bindgen`-generated union / a field read like `unsafe { u.field }` | §B5 (union read requires initialized storage plus a local validity proof for the selected field type; tagged C unions need tag checks, intentional type-punning needs a documented proof) |
...
| `x << n` / `x >> n` with a runtime shift count and no explicit checked/wrapping policy | §B26 (shift-count overflow or masked semantics) |
| `a / b` / `a % b` without a `b != 0` guard | §B26 (div/rem by zero panic) |
| `x << n` / `x >> n` where `n` comes from input/config or an unchecked computation | §B26 (shift-count overflow/masked semantics) |
...
- **Toolchain is part of the artifact.** Record the exact `rustc --version --verbose`, Cargo version, target triple, and any `rust-toolchain.toml`/`rust-toolchain` channel in CI. A stable-channel label alone is not reproducible: patched point releases can change lint, standard-library, or advisory behavior. Re-run this version check after a toolchain update.
...
`cargo audit`/`cargo deny` are release or CI gates, not substitutes for reading the lockfile. If a repository cannot use one of them, record the reason and the compensating advisory/source review in the audit report rather than silently skipping supply-chain checks.
