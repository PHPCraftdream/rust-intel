...
- §B21 any Tokio API returning a `JoinHandle` whose result is dropped — `tokio::spawn`, `tokio::task::spawn_local`/`LocalSet::spawn_local`, `tokio::task::spawn_blocking`, `Handle::spawn`/`Handle::spawn_blocking`, `Runtime::spawn`/`Runtime::spawn_blocking`, and any builder/runtime-handle variant returning the same handle type — not only the bare `tokio::spawn` form
...
| "with timeout", "select!", "cancel", "race two futures", "first one wins" | §B3 cancel safety; §B23 select arm side effects | Silent partial state, no cancel-safe annotation; an unrecoverable-or-duplicated side effect on a losing arm is broken by cancellation — "atomic" alone does not exempt it (a race-free `fetch_add` still double-counts on drop-and-recreate); the actual bar is idempotent, rollback-provable, or commit-observable |
...
| "store a password", "salt the hash", "PBKDF2/Argon2 parameters", "derive a key from a password" | §B12 crypto insecurity | fixed/reused salt; below-OWASP-floor work factors; a fresh random salt per credential via the KDF crate's own generator for the pinned version (`SaltString::generate(&mut OsRng)` on `password-hash` ≤0.5, `generate_salt()`/`try_generate_salt()` on ≥0.6 — the API moved, don't cite one form as universal) + OWASP params |
...
| "scoped threads", "std::thread::scope", "borrow into a thread", "fan out without `'static`" | §B9 thread::scope auto-join | sync mirror of §B21 — children force-joined on the closing brace; a child waiting on a parent resource deadlocks the brace; a child panic re-panics the parent when `thread::scope` returns, after auto-joining every un-joined child at the closing brace (not in any `Drop` impl) |
...
| a `DashMap`/`dashmap::Ref`/`RefMut` from `entry()`/`get()`/`get_mut()`, or an `scc::HashMap` occupied-entry guard from `entry()`/`get()`/`entry_async()`/`get_async()` (sync *or* `*_async` form), bound to a `let` that is still live across a later `.await` | §B2 (map guard across `.await` — a DashMap shard `RwLock` or an scc per-bucket guard blocks/self-deadlocks exactly like a `MutexGuard`; `*_async` yields instead of blocking a thread but the guard is still exclusive, so a second `.await` while holding it can still self-deadlock) |
...
any impl containing an **uncovered** type parameter — per the Rust Reference's own definition: a type parameter is uncovered wherever it appears *without* being nested as an argument to another (local) type, checked across **every** position in the impl header, both the `Self` type and the trait's own generic arguments. `impl<T: Bound> PublicTrait for T`, `impl<T> Bar<T> for T`, `impl<T> Bar<Vec<T>> for T`, and — the case a Self-type-only check misses — `impl<T> Bar<T> for Vec<T>` (covered in `Self`, but `T` is still bare/uncovered in the trait argument `Bar<T>`) are all blanket impls. Contrast `impl<T> Bar<Vec<T>> for Vec<T>`: **every** occurrence of `T` is covered (nested inside `Vec`), so this is an ordinary generic impl, not a blanket one — in the public API of a published library | §C1 (blanket-impl semver hazard — the module's only 🔴 case) |
| a type declared behind a private module ancestor with no `pub use` re-export from the crate root, used anywhere in a public signature — a `pub fn`'s return type or a **parameter** type, a public struct field, a public type alias, or a `pub trait`'s method signature / associated type | §A3 (`unnameable_types` case — the type is reachable through the signature even though no import path names it, in any of these positions, not just a return type; `unreachable_pub` alone does not catch this) |
| a `pub fn`/public method returning a type carrying a non-`'static` lifetime tied to `&self` or a parameter, where the lifetime is not an explicitly documented zero-copy design goal | §B1b (lifetime leaking through public APIs — every downstream caller is forced to thread that lifetime through their own code; default to an owned return type unless zero-copy is a stated goal) |
...
| any Tokio API returning a `JoinHandle` — `tokio::spawn(...)`, `tokio::task::spawn_local(...)`/`LocalSet::spawn_local(...)`, `tokio::task::spawn_blocking(...)`, `Handle::spawn(...)`/`Handle::spawn_blocking(...)`, `Runtime::spawn(...)`/`Runtime::spawn_blocking(...)` — whose returned `JoinHandle` is not bound, not awaited, not detached-by-design | §B21 (dropped JoinHandle ≠ abort — the task already ran and keeps running; it detached, it was not left unpolled; the rule is semantic — *any* API returning this handle type, not an enumerated list of spelling) |
...
| Manual `impl PartialEq` or `impl Ord` (or a manual/derived mix that doesn't agree) on a type used as a `HashMap`/`BTreeMap` key, `Vec::sort*`/`BinaryHeap` element, or `.sort_by` comparator input | §B16 (Eq/Hash contract; an inconsistent `Ord` is a logic error with unspecified — not uniformly silent — outcome across every total-order consumer, not just B-tree keys) |
| `HashMap`/`HashSet` built with `FxHashMap`/`rustc_hash`/`fnv`/a fixed-seed `ahash`/`foldhash`/`hashbrown` `BuildHasher`, or `ahash::RandomState::default()`/`foldhash`'s default (randomized-seed) `RandomState`, where the keys come from request bodies/headers/parsed external data | §B16 (HashDoS — `FxHashMap`/`fnv` are never negotiable on untrusted keys regardless of seeding; `ahash`/`foldhash`'s randomized-default modes are narrower — banned specifically with a fixed/predictable seed, and `foldhash`'s default mode additionally needs the non-interactive/single-shot scoping before it's usable against untrusted keys at all — see the Substitution catalog in `data-and-types.md`) |
| `tokio::select! { ... }` with an unrecoverable-or-duplicated side effect in an arm's polled future expression (before `=>`) — not the handler after it, which runs only for the winning arm; a merely-atomic effect (`fetch_add`, an atomic swap) is **not** automatically exempt — Tokio's drop-and-recreate criterion is idempotence/rollback-provability/commit-observability, not race-freedom | §B23 (arm side effects); also see §B15e for a distinct `select!` hazard this row doesn't cover — re-polling an already-completed arm future across loop iterations, or an all-arms-disabled `select!` with no `else`, both panic |
| `tokio::spawn`/`tokio::task::spawn_local`/`LocalSet::spawn_local` (needs `.in_current_span()`) or `tokio::task::spawn_blocking` (needs an explicitly captured-and-re-entered span; `.in_current_span()` does not help here) inside a function with an active `tracing::Span` | §C9 (span leakage — each spawn form loses the parent span the same way, but the fix differs by form) |
...
| `tokio::sync::mpsc::unbounded_channel()` / `flume::unbounded()` / `async_channel::unbounded()` with no comment proving a finite total-message bound or a finite backlog envelope (§B14's two justification shapes) | §B14 (unbounded channel — no size cap means no backpressure, regardless of how politely the producer is coded) |
| `FuturesUnordered` pushed unbounded or polled while empty in `select!`; a `JoinSet`/`JoinSet<T>` grown by **any** task-insertion method (`.spawn()`, `.spawn_on()`, `.spawn_local()`, `.spawn_local_on()`, `.spawn_blocking()`, `.spawn_blocking_on()`, or a future stable insertion API of the same shape) with no cap and no matching drain (`.join_next()`) | §B14 (busy-loop / unbounded growth — the set-growth hazard applies identically whichever insertion method fills it; bounded or continuously-drained use is not the defect) |
...
| a regex/tag blocklist applied to untrusted content before HTML output (`SCRIPT_RE.replace_all(input, "")`, stripping only `<script>`); hand-written entity escaping (`input.replace('<', "&lt;")` and similar ad hoc `replace` chains, not an auto-escaping template engine); `pulldown_cmark::html::push_html`/any Markdown-to-HTML renderer fed untrusted input with no sanitization pass (`ammonia` or equivalent) on the rendered output and no URL-scheme allowlist on link/image targets | §C12 (🔴 — the three named XSS-shaped rows: hand-rolled HTML sanitization, Markdown-to-HTML rendering, and HTML escaping; a blocklist/ad hoc-escape/unsanitized-renderer shape is the canonical silent bypass for each) |
...
| `std::thread::scope(\|s\| { s.spawn(...) ... })` whose children await a resource the *parent* code after `s.spawn` would release | §B9 (scope auto-joins on closing brace → deadlocks; a child panic re-panics the parent when `thread::scope` returns after that auto-join, not in any `Drop` impl) |
...
| `regex::Regex::find_iter`/`captures_iter`/`replace_all` (the **linear** engine) over a fully-untrusted, unbounded-length haystack with no cap on matches processed; or `Regex::new`/`RegexBuilder::new` compiling a pattern built from untrusted input with no `size_limit`/pattern-length cap | §B16 (the linear engine's `find_iter`/`captures_iter` are `O(m·n²)` worst case per the crate's own docs — not the `O(m·n)` single-search bound — cap the match count; an untrusted pattern needs `size_limit` regardless of engine) |
...
| a new entry in any `Cargo.toml` dependency table — `[dependencies]`, `[dev-dependencies]`, `[build-dependencies]`, a `[target.'cfg(...)'.dependencies]` table, or `[workspace.dependencies]` — registry, path, renamed (`package = "..."`), or otherwise, not just a `[patch]`/git source | §A1 (every new dependency is the slopsquatting/unverified-crate finding regardless of table or dependency kind — classify it by whether the crate was user-named, independently verified to exist, and pinned; the `[patch]`/git row below is the *unpinned-source* sub-case of this same category, not the only trigger for it) |
...
- **`std::assert_matches!`** per §D1 — stable since **Rust 1.96**, above this spec's declared MSRV floor of 1.85; not in the prelude, needs `use std::assert_matches;` or the fully-qualified `std::assert_matches!(...)`. On a project pinned below 1.96, `assert!(matches!(value, pattern), "unexpected value: {value:?}")` is the fallback **only when `value: Copy` or the pattern binds nothing by value** — if the pattern captures a field by value (e.g. `Ok(x)` where the payload isn't `Copy`), `matches!` moves the captured part out of `value`, and the `{value:?}` diagnostic then borrows a partially-moved place (`E0382`) even though the assertion looks correct. Match `matches!(&value, pattern)` instead (adjusting the pattern to match through the reference) when `value` must still be usable, or capture whatever the diagnostic needs into a separate binding first.
