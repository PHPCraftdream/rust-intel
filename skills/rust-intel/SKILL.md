...
- §C12 (the hand-rolled HTML-sanitization and Markdown-rendering rows only — both an XSS-shaped silent bypass; the rest of §C12/§C12a is 🟡, applied at write time)
...
| "parse CSV/JSON/URL by hand", "write my own parser for", "quick script, no dependencies", "avoid adding a crate for this", "just use split/format!" | §C12/§C12a reinventing a solved problem | a naive `split`/`format!`/regex shape passes an easy test and is silently wrong on a real input — propose the catalog's named crate (per §A1) before hand-rolling |
...
`cargo audit`/`cargo deny` are release or CI gates, not substitutes for reading the lockfile. If a repository cannot use one of them, record the reason and the compensating advisory/source review in the audit report rather than silently skipping supply-chain checks. `cargo semver-checks` is the same kind of gate for §C1/§C1a/§A3: it mechanically catches the delayed-blast forms those categories describe (a blanket impl narrowing, a missing `#[non_exhaustive]`, a trait method addition that breaks object-safety or drops an auto-trait impl) before they reach a consumer's CI — but it documents its own gaps (generics, lifetimes, feature/target-specific breakage), so the categories' write-time discipline still carries what it doesn't cover.
