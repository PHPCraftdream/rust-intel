...
- `parking_lot::Mutex` (no poisoning by design) does not repair a broken invariant — it removes the *signal*, not the corruption. It's a sound choice only when no cross-field invariant can be left half-updated by a panic; otherwise it trades a loud panic for silent, undetected data corruption.
...
- `futures::executor::block_on` is a **different function from a different crate** and knows nothing of tokio's runtime context — it has no built-in re-entrancy/nesting check the way tokio's does, so calling it does not itself panic on nesting — but a panic inside the awaited future still propagates through it normally, same as any sync call. On a multi-thread runtime it usually completes by luck (the timer/I/O it awaits is driven by another worker); on `current_thread` it typically deadlocks silently — no panic, no error, just a hang — unless the awaited future can complete without needing the parked reactor to make progress (e.g. it's already resolved, or depends on no tokio-driven I/O/timer). That is the worse outcome in the deadlocking case: the opposite of the loud failure a test would catch.
