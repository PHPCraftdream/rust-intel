...
- Shift operators `x << n` / `x >> n` where the shift count is untrusted or computed without an explicit policy. Counts outside the type's bit width overflow; with overflow checks disabled, behavior is not the checked error a caller may assume. Use `checked_shl`/`checked_shr`, or document and test the deliberately wrapping/masked semantics.
