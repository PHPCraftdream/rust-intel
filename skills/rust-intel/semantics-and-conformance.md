...
- `.read()` / `read_exact` / `next()` on a stream from an untrusted peer without an **absolute** whole-frame/whole-handshake deadline or a minimum-progress-rate check — a fresh `timeout(dur, read_exact(..))` around each individual read is *not* such a mechanism (see the REQUIRED list below for why), and neither is a resettable idle timer used *alone*: a client sending one byte just before each idle window expires resets it indefinitely at near-zero rate, and no read is ever silent long enough to trip it. (Connect-then-silence is the cheapest DoS there is; a trickle is the same DoS wearing a slower disguise. 🔴 when peer-extendable.)
...
- An **absolute** deadline on every untrusted-peer read stage, documented together with what happens on expiry. The deadline shape matters: a whole-frame/whole-handshake deadline (one fixed budget covering the entire frame or handshake, counted from when that stage started — it does **not** reset on partial progress) or a minimum-progress-rate check (a required byte count per unit time) — never a fresh timeout around each individual read, and never a plain idle/read timer that resets on any byte received, however small or slow. Both of those defeat only *silence*: a peer emitting one byte just before each per-read (or idle) window expires keeps every window from ever firing, so the connection stays open forever at whatever trickle rate it likes (slowloris-shaped). An idle timer is still useful, but only as an *additional* policy layered on top of the absolute deadline (e.g. closing an established, otherwise-healthy long-lived connection that goes fully silent) — never as the sole or alternative defense.
