# Rust Intel — Semantic Conformance

> Module of the **rust-intel** skill. Core lives in `SKILL.md`. This module holds the category bodies for §F1–§F4 — defects of *meaning*: code that compiles, passes its own tests and clippy, and implements the wrong thing. These categories are not found by pattern-matching; see SKILL.md → "Tier F — how to review for meaning".
> **Tiers in this module:** §F1 🔴/🟡 · §F2 🔴/🟡 · §F3 🟡 (🔴 when attacker-extendable) · §F4 🟡. Derived from SKILL.md → Enforcement tiers (canonical); the conditional-🔴 rule is restated in the "Tier F — how to review for meaning" section there.
> **Audit semantics:** unlike Tiers A–E, the auditable artifact here is often an *absence* (a spec-mandated state not handled, a round-trip test not present, a documented guarantee not enforced). An absence finding must name the reference it is absent *from* — "the spec requires X (section/source), the code has no path for X" — never a bare "looks incomplete".

---

## §F1. Spec / reference conformance — *self-consistent, green, and wrong*

**The trap**: the code names an external source of truth — an RFC, a file format, a protocol, "a port of <upstream>" — and then diverges from it: a wire field emitted in the wrong order or width, an endianness flipped, a length prefix counting bytes where the spec counts elements, a state machine handling the common transitions and silently absorbing the rest in `_ =>`, an error mapped to the wrong protocol code. Every internal test passes, because the tests were written against the same misreading. The bug surfaces only against a *real* counterpart — by then the wrong format may already be persisted or deployed on both ends.

**BANNED**:
- Implementing a named spec/format from memory of how it "usually works", without the reference text (or reference implementation) at hand. A protocol detail recalled is a protocol detail guessed.
- A `match` on a protocol state / opcode / message-type with a `_ => ` (or `_ => unreachable!()` / silent-ignore) arm covering values the spec defines and assigns behavior to. Wildcards are for values the spec itself leaves reserved — and then the spec's mandated reserved-value behavior (reject? ignore? echo?) is what the arm must implement, with a comment citing the section.
- Verifying conformance by round-tripping against your own implementation only (`assert_eq!(decode(encode(x)), x)` proves inverse-consistency — §F4 — not conformance; both halves can share the same misreading).
- "Compatible with X" in docs/README when no test exchanges bytes with X itself, X's reference implementation, or X's published test vectors.
- Silently extending or restricting the spec ("we also accept...", "we never send...") without a `// DEVIATION from <spec §>: <what and why>` comment. Undocumented deviation is indistinguishable from a bug.
- Treating **your crate's own previously-released encoding** as exempt from conformance. For a positional, non-self-describing format — `bincode`, `postcard`, `rkyv`, `borsh` under their **default, derived** encoding: enums as a bare variant index in declaration order, structs as unnamed fields in order — the byte layout of every released version *is* a spec that already-persisted data depends on. (`borsh` specifically also supports **explicit** discriminants — `#[borsh(use_discriminant = true)]` and per-variant `= N` values — which decouple the wire **variant tag** from declaration order; that exemption covers only variant insertion/reordering when every variant keeps its own unique explicit tag unchanged, and nothing more — it does **not** stabilize the declaration order of fields inside a struct-like/tuple-like variant's payload, nor the field order of a top-level struct, so payload/struct field-order changes still need the corpus/version protections required below. Inserting/removing a variant is still a compatibility decision to make deliberately, just not this particular silent one. Check whether the type in front of you uses default ordinal encoding or explicit tags before applying this rule.) Inserting a new enum variant "in a sensible place" (not at the end) or reordering fields can silently reinterpret every stored value of a later variant as the new one whenever the two variants' payload shapes happen to be positionally compatible (`Created`, `Deleted`, then `Updated` inserted between them → every stored `Deleted` now decodes as `Updated` if both are unit-like or otherwise byte-compatible at that position) — when the shapes are *not* compatible (different field types/counts), decoding fails outright instead, which is at least visible rather than silent. Either way the §F4 round-trip property stays **green** on the compatible-shape case: both encoder and decoder were regenerated from the same new declaration, so the test never exercises old, already-stored bytes. The same class wears a serde-free face: a `#[derive(PartialOrd)]` field reorder silently changes ordering semantics — declaration order is load-bearing. REQUIRED: treat the crate's own previous release as a reference spec — cite the version whose layout you must stay conformant with, ship a golden-bytes decode **corpus** (not one fixture) captured from the prior release, and gate any variant/field insertion, reorder, or removal on that corpus still decoding correctly. One fixture proves nothing about the variants/field-values it doesn't cover — the corpus must cover the **finite graph of distinct serialized type/variant definitions** reachable from every persisted root type, walked to a fixed point and visiting each distinct type/variant definition once (once per relevant monomorphization for generics) — not once per runtime occurrence: **every top-level `struct`'s fields, every nested `struct`'s fields, and every enum variant of every enum in that graph** (a variant of an `Inner` enum reachable only through a top-level `Record { inner: Inner }` field still reorders silently if the corpus holds only `Record { inner: Inner::A }`; a struct has no variants, so an "every variant" corpus is vacuously empty for one — yet a field reorder of same-typed fields silently exchanges every stored value's meaning: `struct Pair { left: u32, right: u32 }` re-declared as `{ right, left }` swaps every stored pair). A **recursive edge** in that graph is real, not a type-system corner (`struct Node { value: u8, next: Option<Box<Node>> }` — borsh ships `BorshSerialize` for `Option<T>` and `Box<T>`) and is covered by exactly **two representatives, not every runtime depth**: one terminating (the base case, `next: None`) and one recursive (one level of actual recursion, `next: Some(Box::new(Node { .. }))`) — a literal "every variant at every nesting depth" reading demands an unbounded corpus (depth 1, 2, 3, …) that no finite corpus can satisfy; do not assume a format-level depth limit the pinned format's own docs do not state. Coverage is necessary but not sufficient — **the oracle is decode-observable, not typed-value-level**: for every non-identity permutation of a group of mutually-reorderable sibling positions (positions that could swap under the type system — same type, or wire-compatible types), at least one prior-release fixture must, when its golden bytes are decoded under the **proposed (reordered) layout**, either **fail to decode** or **decode to a value that differs from what that fixture's named-field expectation says it should be**. Distinct **typed** values prove nothing about this: different Rust types can encode to identical wire bytes (bincode 2's spec: `bool` is one byte `0`/`1`, `u8` is itself, and struct fields serialize in declaration order with no names — so `struct S { n: u8, flag: bool }` with fixtures `{ n: 0, flag: false }` and `{ n: 1, flag: true }` has "distinct typed vectors" but both positions' wire byte-vectors are `[0, 1]`, and after reordering to `{ flag, n }` both old records still decode to their original logical values). Practical construction: choose values whose **encoded byte signatures** are distinct across every wire-compatible sibling group — a `bool` sibling is locked to bytes `0`/`1`, so a `u8` sibling needs values outside `0`/`1` for the two positions' byte-vectors to differ (with `{ n: 2, flag: false }` and `{ n: 3, flag: true }`, the reordered layout fails to decode outright: `2` is an invalid `bool` per bincode's spec); when every sibling's domain is the same restricted set, hand-picking may be impossible — **so prefer the stronger, more mechanical schema-mutation negative control**: actually construct the reordered/mutated schema, decode each prior-release fixture's golden bytes with it, and assert the result fails to decode or differs from the original schema's decoded value — that tests the detectability property directly instead of approximating it through chosen values. (Same-type positions keep the simpler corpus-level rule: a single record cannot always separate positions — for `struct S { a: bool, b: bool, c: bool }`, `true` is the only non-default value, so `S { a: true, b: true, c: true }` is the only fully-non-default record and detects no swap, while a three-fixture one-hot corpus (`{ true, false, false }`, `{ false, true, false }`, `{ false, false, true }`) gives each position unique encoded bytes and detects every swap) (a one-fixture corpus of `Pair { left: 0, right: 0 }` gives both positions the same vector and stays green through a swap; `Pair { left: 1, right: 2 }` gives distinct vectors and does not) — not just any value from that variant/struct; a genuinely singleton/zero-byte position (`()`, `PhantomData<T>`, a one-variant unit-like enum) has no distinct marker value to place — there the corpus instead pins the position's exact golden bytes (only one legitimate value exists, so any decode that does not reproduce them is already wrong) and distinguishes the neighboring meaningful positions, rather than demanding an impossible marker at the singleton position; for anything with more than one deployed layout, carry an explicit format/version byte versioned from the format's **first released layout** (or migrate old unversioned data behind unambiguous external metadata or a magic-byte sequence that could not occur in the old format — a single small version byte retrofitted onto previously-unversioned data can collide with an old record whose first data byte already equals it, so the record misclassifies as a recognized version and the rejection policy never even runs) **plus a stated migration/compatibility/rejection policy, with per-version decode dispatch, covering every deployed layout the code claims to support** — not merely detection of the unrecognized case; a checked version byte with no such policy can still silently discard or misdecode data under a documented compatibility promise, it just fails one release later than a missing byte would. `prost`/`capnp` used per their own documented evolution rules are the designed-for-this alternative.

**REQUIRED**:
- Cite the reference at the implementation site: `// RFC 9293 §3.5.2` / a link to the upstream function being ported. Uncited conformance claims are unreviewable.
- Test against the *external* oracle wherever one exists: published test vectors, golden byte-fixtures captured from the reference implementation, or an interop test against the real counterpart. Fixtures must be captured/derived from the reference, not generated by the code under test (that is the §D1a circular oracle).
- For state machines: enumerate the spec's states and transitions in a table (comment or doc) and ensure each row has a code path and each spec-mandated rejection is tested. The enumeration is the review artifact; "looks complete" is not.
- On review (audit mode): list spec-mandated behaviors first, tick off against the code, report the unticked. See SKILL.md → Tier F stance, rule 3.

**Example — compiles, green, wrong**:
```rust
/// Encodes a length-prefixed frame "per the wire spec".
fn encode_frame(payload: &[u8]) -> Vec<u8> {
    let mut out = Vec::with_capacity(payload.len() + 2);
    out.extend_from_slice(&(payload.len() as u16).to_le_bytes()); // spec says big-endian
    out.extend_from_slice(payload);
    out
}
// decode_frame reads to_le_bytes too — round-trip test green, clippy silent,
// interop with any conforming peer: every frame length is byte-swapped.
```

## §F2. The project's own documented guarantees — *the bug is only visible after reading the README*

**The trap**: the project's README / SECURITY.md / design doc states guarantees in prose — "tokens are never logged", "all input on this port is untrusted", "writes are durable after `commit` returns", "events are delivered in order per key" — and a diff, locally reasonable, violates one: a new `debug!("{request:?}")` where the request holds a token; a parser on the untrusted port that trusts a length field; a fast path that returns before `fsync`. No category in Tiers A–E fires, because each guarantee is project-specific. The defect exists only in the gap between the code and a document the reviewer didn't open.

**BANNED**:
- Reviewing or modifying code in a project with stated guarantees without first extracting those guarantees into an explicit checklist (SKILL.md → Tier F stance, rule 4). This is a process ban with an auditable artifact: a Tier-F review that cites no project doc has not performed §F2.
- Weakening a documented guarantee "internally" (the doc still promises it) — e.g. adding a config flag that disables an invariant the README states unconditionally — without updating the doc in the same change.
- Treating data as trusted because the *code path* looks internal, when the *docs* classify its origin as untrusted (the doc, not the call graph, defines the trust boundary).
- New logging/telemetry/error messages formatting types that the project's docs classify as sensitive (see also §B12 Debug-leak — that category catches `password`/`token` field *names*; this one catches what the *docs* declare sensitive regardless of name).

**REQUIRED**:
- Before generating or reviewing code in such a project: read the guarantee-bearing docs, list the guarantees touched by this change, and state in the response which were checked ("guarantees checked: ordering-per-key (unaffected), no-token-logging (touched — see line N)").
- When a change makes a documented guarantee false, the doc change ships in the same diff — or the code change is rejected.
- When the docs and the existing code already disagree, that is a finding in itself (report it; don't silently pick a side).

**Example — compiles, green, wrong**:
```rust
// README: "the bridge treats everything received from the client socket as
// untrusted and never allocates based on client-supplied sizes."
let n = read_u32(&mut client).await?;          // client-supplied
let mut buf = Vec::with_capacity(n as usize);  // violates the documented guarantee
// §B7's trigger needs the reviewer to *know* `n` is untrusted; only the README says so.
```

## §F3. Resource lifecycle at the boundary and on the error path — *the happy path cleans up; the `?` path leaks*

**The trap**: connection/stream/file handlers are written happy-path-first: cleanup (shutdown, flush, unregister, decrement, abort the paired task) sits at the function tail, and every early `?` / `return` / `break` skips it. Or EOF handling: `read` returning `Ok(0)` is looped on (busy-spin) or treated as an error instead of a half-close; the write side is never `shutdown()`, so the peer waits forever. Or no read deadline on an untrusted remote: a peer that connects and sends nothing pins a task, a buffer, and an FD indefinitely. Tests are green because the test peer is well-behaved, fast, and always closes politely. Production peers are none of these.

**BANNED**:
- Cleanup that runs only on the `Ok` path — any teardown reachable from fewer paths than the acquisition. If the resource needs teardown, it needs an RAII guard (§B4) or a single exit funnel; "I'll remember to clean up before each `return`" does not survive the next `?` someone adds.
- `.read()` / `read_exact` / `next()` on a stream from an untrusted peer without an **absolute** whole-frame/whole-handshake deadline, or a minimum-progress-rate check that is itself bounded by a finite maximum stage size or an explicit maximum duration — a fresh `timeout(dur, read_exact(..))` around each individual read is *not* such a mechanism (see the REQUIRED list below for why), neither is a resettable idle timer used *alone* (a client sending one byte just before each idle window expires resets it indefinitely at near-zero rate, and no read is ever silent long enough to trip it), and neither is a minimum-rate check with no cap: an unknown-length stage sent at exactly the minimum acceptable rate forever still never trips a rate-only check, holding the task/socket/buffer open indefinitely. (Connect-then-silence is the cheapest DoS there is; a trickle at the rate floor is the same DoS wearing a slower disguise. 🔴 when peer-extendable.)
- Treating `read() == Ok(0)` as anything other than EOF: retrying it (busy-loop), erroring on it when the protocol allows half-close, or tearing down the write direction that may still have data to flush. (Qualification: `Ok(0)` means EOF only when the destination buffer was non-empty — std's `Read::read` docs list "the buffer specified was 0 bytes in length" as an independent second scenario for `Ok(0)` — so `.read(&mut [])` legitimately returns `Ok(0)` with no EOF implication; don't apply this rule to a zero-length-buffer call.)
- In a bidirectional byte proxy: an unbounded buffer between the two directions (§B14 applies — the slow side must backpressure the fast side, not grow a Vec), and copy loops that don't propagate shutdown — when one direction EOFs, the proxy must `shutdown()` the corresponding write side, not just drop the loop.
- Spawning a per-connection sibling task (heartbeat, writer half) without aborting/joining it on every exit path of the main handler — the `Err` path included (§B21 twin, scoped to the connection lifecycle).

**REQUIRED**:
- For every acquired boundary resource, name the release point and verify it is reached from *every* exit: each `?`, each `select!` arm losing the race (§B3/§B23), panic (does Drop suffice?). The review move is path enumeration, not pattern recognition.
- A stage-lifetime cap that is genuinely finite either way it is expressed: a **whole-frame/whole-handshake deadline** (one fixed budget covering the entire frame or handshake, counted from when that stage started — it does **not** reset on partial progress), **or** a minimum-progress-rate check **combined with** a finite maximum stage size (a rate floor over a bounded size bounds duration) or an explicit maximum duration on its own — a bare minimum-rate check with neither is not itself a bound: a peer sending an unknown-length stage at exactly the rate floor forever satisfies the check on every measurement and never trips it. Document what happens on expiry either way. Never a fresh timeout around each individual read, and never a plain idle/read timer that resets on any byte received, however small or slow — both defeat only *silence*: a peer emitting one byte just before each per-read (or idle) window expires keeps every window from ever firing, so the connection stays open forever at whatever trickle rate it likes (slowloris-shaped). An idle timer is still useful, but only as an *additional* policy layered on top of a genuinely finite stage bound (e.g. closing an established, otherwise-healthy long-lived connection that goes fully silent) — never as the sole or alternative defense.
- A fired read deadline is TERMINAL for the connection — close it; never retry the read in place. This is what makes a deadline wrapped around `read_exact` safe despite its cancel-unsafety (§B3): `tokio::time::timeout` cancels (drops) the inner future on expiry, and a cancelled `read_exact` **may** already have read some bytes into the buffer passed to it — the buffer itself is caller-owned, not owned by the dropped future, but the *count* of how many of its bytes are now valid is unknown to the caller. Tokio's own docs: "This method is not cancel safe... some data may already have been read into buf" — so a retry resumes misaligned, in the middle of a partially-consumed frame it cannot measure. If reads must survive cancellation instead, use the cancellation-safe `read()` (never `read_exact`: tokio docs — "it is guaranteed that no data was read") into a persistent framing buffer that resumes without losing consumed bytes.
- Explicit EOF policy per stream: on `Ok(0)` — flush and `shutdown()` write side? close both? Document it; "whatever the code happens to do" is where the bug lives.
- Tests must include the rude peer: connects and stalls, sends a partial frame and dies, half-closes mid-stream. An in-memory duplex that always behaves is the §D1a stub-oracle trap.

**Example — compiles, green, wrong**:
```rust
async fn handle(mut client: TcpStream) -> io::Result<()> {
    let upstream = TcpStream::connect(UPSTREAM).await?;
    register_conn(&client).await;            // metrics: conn count +1
    let creds = read_handshake(&mut client).await?;  // early `?`: +1 never unregistered; no graceful close of upstream
    proxy(client, upstream, creds).await?;           // until Drop, conn count never -1:
    unregister_conn().await;                         // skipped on every error path
    Ok(())
}
// Tests use a polite in-memory peer: handshake always succeeds, count always balanced.
// In prod, a scanner that connects and disconnects drifts the gauge upward forever.
// Not a socket leak: the owned `TcpStream`s drop and close on the early return.
// The real defects: the +1 registration never balances (unmatched unregister) and
// upstream gets no graceful protocol shutdown (close frame/half-close) — just an abrupt drop.
```

## §F4. Property obligations for inverse pairs — *encode/decode written together, never proven inverse*

**The trap**: the codebase ships an inverse pair — `encode`/`decode`, `parse`/`Display`, `to_bytes`/`from_bytes`, `encrypt`/`decrypt`, `compress`/`decompress` — and tests each half on a handful of literals, never asserting the round-trip law over the input domain. The halves drift: an escape added to `Display` that `parse` doesn't strip, a field appended to `encode` that `decode` skips, padding `encrypt` adds that `decrypt` truncates wrong on boundary sizes. Each half's unit tests stay green; only the *composition* on an unanticipated input is broken. (§B20 mandates this for serde; this category generalizes it to every hand-written inverse pair — where no framework reminds you, so it's most often missing.)

**BANNED**:
- Shipping one half of an inverse pair without a round-trip test of the composition (`decode(encode(x)) == x`, or for parse/Display: `parse(display(x)) == x`) in the same change. The pair is one unit; testing the halves separately tests two functions and zero laws.
- Round-trip over literals only, when the input domain has structure: empty input, max-length, every escape-worthy character, boundary sizes (block size ± 1 for crypto/compression), non-ASCII/UTF-8 multi-byte (§B28), `Option`/default-collapsing values (§B20). Hand-picked literals encode the author's blind spots — but a `proptest::Strategy` is author-written too and can encode the same blind spots (e.g., a strategy that only generates ASCII when the real bug is in non-ASCII handling): review the strategy's own coverage, and keep the explicit escape/boundary/invalid-input corpora alongside the generated cases rather than letting generation replace them.
- Asserting the round-trip in the wrong direction only. `decode(encode(x)) == x` (full domain of `x`) and `encode(decode(b)) == b` (canonical-form question — often *legitimately false* when multiple encodings normalize) are different laws; know which one your format promises and test that one. Don't let a failing canonical-form test get "fixed" by weakening the real law.
- Letting a round-trip property substitute for §F1 conformance: round-trip proves the pair agrees with *itself*, not with the spec. Both are required for a wire format; neither implies the other.
- Citing a green `prop_decode_encode_roundtrip` as proof that persisted data still decodes after a variant/field insertion or reorder in a positional format (`bincode`/`postcard`/`rkyv`/`borsh`) — both halves of the pair were regenerated from the same new declaration, so the property cannot observe the stored bytes it broke. The §F1 golden-bytes corpus captured from the prior release is the negative control this property lacks.

**REQUIRED**:
- One `proptest`/`quickcheck` round-trip property per inverse pair, named for the law (`prop_decode_encode_roundtrip`), generating over the realistic input domain — shipped in the same change as the pair (Pre-flight item 9).
- For fallible decode: also a corpus of *invalid* inputs asserting `Err` (a decoder that never rejects accepts garbage — the negative control, §D1a).
- For `FromStr`/`Display` pairs: the round-trip property is the API contract *only when the type's own docs establish it* — i.e. `Display` is documented as lossless and machine-parseable. There, `x.to_string().parse() == Ok(x)` is the law users assume; breaking it is a semver-relevant behavior change — flag inline. Where `Display` is documented as human-readable/lossy output, no round-trip guarantee applies — std explicitly permits `Display` to be lossy or unparseable — so test the law the type's docs actually state (if any) instead of leaving the pair silently untested.
- When the pair is intentionally non-inverse (lossy compression, normalizing parser), state the actual law in a doc comment and test *that* (`parse(display(parse(s))) == parse(s)` — idempotence) instead of silently testing nothing.

**Example — compiles, green, wrong**:
```rust
impl Display for Tag { // "key=value;key=value"
    fn fmt(&self, f: &mut Formatter) -> fmt::Result {
        write!(f, "{}={}", self.key, self.value) // value may contain '=' or ';'
    }
}
impl FromStr for Tag {
    fn from_str(s: &str) -> Result<Self, ParseError> {
        let (k, v) = s.split_once('=').ok_or(ParseError)?; // first '=' wins
        Ok(Tag { key: k.into(), value: v.into() })
    }
}
// Unit tests: Display tested on ("env", "prod"), FromStr on "env=prod". Both green.
// Tag { key: "k", value: "a=b" } round-trips fine — the value's '=' survives intact. But
// Tag { key: "a=b", value: "c" } round-trips to Tag { key: "a", value: "b=c" }: the
// key's '=' is exactly where "first '=' wins" breaks — and
// no test composes the two halves over generated values, so nothing ever fails.
```
