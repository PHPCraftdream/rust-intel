...
- `Vec::into_raw_parts` — **stable since Rust 1.93** — verify against your toolchain; treat as unstable below a confirmed 1.93 and use the `ManuallyDrop` form. For MSRV < 1.93, use the `ManuallyDrop<Vec<T>>` + manual `(ptr, len, cap)` decomposition (stable since 1.0). The spec's MSRV floor is 1.84, so by default the manual form is what you write; bump the MSRV explicitly if you want the convenience.
