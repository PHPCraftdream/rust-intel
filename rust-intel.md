...
Treating all 50 categories as equally critical produces noise that buries the few findings that matter. Apply rules at one of three tiers:
...
- **Strict provenance (Rust 2024 edition)**: prefer the strict-provenance API (`ptr.with_addr(addr)`, `ptr.addr()`, `ptr.map_addr(|a| …)`, `ptr.expose_provenance()`, `core::ptr::with_exposed_provenance::<T>(addr)`, and `core::ptr::dangling::<T>()` / `core::ptr::without_provenance::<T>(addr)` for a fresh pointer carrying no provenance) over the legacy `ptr as usize` and `usize as *const T` casts. The cast form loses provenance information that miri can otherwise verify; under strict-provenance lints the cast is a warning, and under sanitizers it is unsound. (All of these are stable since Rust 1.84.)
