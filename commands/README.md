...
```
skill/ (SKILL.md core + theme modules + references/sources.md + audit-project.workflow.js)
       │                                              knowledge + fan-out audit process
       ▲
       │ invoke skill
       │
┌──────┴─────────┬──────────────┬─────────────────┐
...
The one apparent exception is `skills/rust-intel/` — a byte-identical **derived** mirror of `skill/`, checked in only because the Codex plugin manifest requires a `skills/<name>/` layout and a git-based install has no build step. It is generated by `npm run sync` (`dev/sync-mirror.mjs`) and its identity is enforced in CI; it is a build artifact that happens to live in git, not a second source of truth. Never edit it directly.
...
The installer also handles the skill (`skill/**/*.md` + `**/*.js` → `skills/rust-intel/`, modular, including `references/`) and sweeps every prior layout at the target — v0.2.1+ flat-with-prefix, v0.2.0 namespace dir, and the v0.1.x flat-no-prefix layout.
