...
**MINOR compatibility change:** npm installation and repository tooling now require **Node.js 24.0.0 or newer**, enforced at startup as well as declared in package metadata. The existing **59 numbered categories**, installer entry points, and installed paths are unchanged. Upgrade Node.js before using the npm installers on a previously supported older runtime.

- **Specification correctness and source alignment.** Audited and repaired security/crypto, async cancellation and shutdown, unsafe/FFI, ownership, concurrency, Serde/API compatibility, Cargo features/macros, and test-oracle guidance. Eight additional consistency passes closed cross-module contradictions and made preconditions explicit; the source ledger, core skill, command adapters, and generated Codex mirror were aligned.
- **Ownership and performance.** Added conditional guidance for `mem::take`/`replace`, buffer reuse, shared-slice retention, capacity growth, sparse-map scans, partial top-k selection, cached sort keys, and reusable records. Failure/restoration, ordering, retention, and input-budget requirements accompany the optimizations.
- **Install and release recovery.** Node, Bash, and PowerShell installers use staged inventories and restartable transactions. Fixed rollback/recovery defects, including PowerShell uninstall deleting a pre-existing foreign skill directory. Version manifests stay synchronized, and publishing verifies the committed version instead of rewriting the tagged tree. Windows recovery is tested for process interruption, not sudden power loss.
- **Validation and CI.** Hardened the repository-specific Markdown/JavaScript scanners, immutable workflow metadata checks, memory/work bounds, execution registry, and anti-vacuity controls. CI covers the exact Node `24.0.0` floor, Windows validation, Bash 3.2, both PowerShell runtimes, and installer recovery matrices.
- **Release rollover.** Fixed the validator rejecting the documented empty `Unreleased` section after a release cut. It now checks a populated `Unreleased`, or the latest dated release matching the package version. Six new regression controls cover that selection and its failure paths; README and all three version manifests are updated for `0.7.0`.
- **npm Trusted Publishing.** Replaced the publish token with GitHub Actions OIDC. Publication now starts when a stable GitHub Release is published, not on tag push alone. The workflow checks the exact release commit against successful CI, enforces stable tag/manifest agreement and npm's OIDC runtime floor, and retains provenance and integrity-aware retry protection. README documents the npm connection fields and release sequence; drafts and prereleases do not publish.

**Release evidence.** Round 54 closed the reviewed P0/P1 blockers; CI run [34324764342](https://github.com/PHPCraftdream/rust-intel/actions/runs/34324764342) passed all 10 jobs at `886f53a`. That run predates the subsequent spec-consistency passes and release preparation: the exact release commit must pass the full validation/recovery workflow before tagging. Preparing this entry does not itself publish a tag or npm package.

### Detailed changes and review history

The entries below preserve the development and review chronology. A round's counts, timings,
open findings, and pending-review statements describe its named revision, not the final release
state; later fixing dispositions and reports supersede them. The current count is the single
unqualified claim in **Net tooling state** below. Underlying reports live in `docs/reviews/`
in the repository, not in the npm tarball.
...
**This release is `0.7.0` (MINOR).** Raising the runtime/install floor makes previously working installs on older Node lines fail, so it is classified by the compatibility rule above even though the numbered spec categories and BANNED/REQUIRED shapes are unchanged.

**Semver classification against `v0.6.0`.** The release boundary is the last released tag, `v0.6.0` (`d5b15ec`), not an intermediate green CI commit. Against that baseline, the Node engine floor changes from `>=16` to `>=24.0.0` and is enforced at startup by the npm installers and validators. The two npm bin targets, installer CLI arguments, installed paths, and 59-category numbering remain unchanged. The runtime compatibility change determines the MINOR classification; earlier point-in-time diff statistics remain in the review reports.
...
case. The round-33 and round-34 fixing dispositions are recorded in the review ledger.
The release workflow verifies that all three manifest versions match the tag before publication.

**Installer and recovery net state.** Node, Bash, and PowerShell installers stage complete
inventories, preserve byte-aware rollback safeguards, and recover interrupted install/uninstall
transactions. Boundary-matrix tests exercise same-operation and cross-operation recovery,
independent replacement inventories, recursive artifact cleanup, and foreign-directory
preservation. The last Windows preservation P1 was fixed by `09bb180` and accepted in round 54;
subsequent CI at `886f53a` passed all 10 jobs. Finite child/job timeouts and the 45-minute publish
timeout bound the checks. Ambiguous recovery states are retained for manual recovery rather than
guessing which paths may be removed. The Windows contract remains process interruption, not
sudden-power-loss durability; historical reports retain the defects and partial fixes that led
to this state.
...
**Focused-lexer anti-vacuity gate rebuilt as behavioral differentials; `dev/validate-lexer-observations.mjs` retired.** Six consecutive independent review rounds (43–47; `docs/reviews/latest-commits-review-round-{43,44,45,46,47}-*.md`) each demonstrated a genuine, empirically working bypass of the gate meant to prove that the fixture's control-401 evidence — a 2,000,000-unit scan by `dev/js-lexer.mjs` — is produced by executing that scan rather than by the probe vehicle, and every one exploited the same structural weakness: a parallel self-reporting oracle (`dev/validate-lexer-observations.mjs`) whose result the parent checked via source-text heuristics on the probe vehicle — regex pins, occurrence counts, position pins — so arranging text around the heuristic (comments, dead branches, wrappers, aliasing) defeated the gate without executing anything more. Following a dedicated redesign consultation, the mechanism is replaced wholesale and the observation module is retired deliberately, not silently dropped: its `required`-file entry, its `node --check` reference, the `RUST_INTEL_CONTROL401_MIN_PEAK_RSS_MB` knob, the telemetry work floors, and the source-inventory pins are removed with it. Every anti-vacuity assertion is now a behavioral differential: the same probe vehicle — byte-identical in both trees, so its own source text can distinguish nothing — runs against (a) the real `dev/js-lexer.mjs` and (b) a temp-tree copy whose `dev/js-lexer.mjs` itself carries a run-time-chosen mutation, and the parent requires exactly the outcome only genuinely executing the mutated spot can produce. Control ids are reused, not renumbered: 401/402 (unmodified tree accepts the exactly-2,000,000-unit marker input at the marker's index / rejects one unit above), 458 (budget lowered by one ⇒ rejection), 459 (a throw injected at a run-time-random operation count K inside `step()` with the K-specific message and index — unreachable for a facade that never calls `step()`, including one that reads the constant and compares lengths), 491 (budget raised by one ⇒ the 402 input accepted), 492 (completion identifier renamed ⇒ no diagnostics), and new 493/494 (block-comment masking liveness: a decoy call inside a comment is suppressed in the real tree and detected when the scanner's comment branch is disabled). The suite moves to 494 controls — 419 child-process (390 validator-entrypoint plus 29 focused) plus 75 in-process — re-derived from the registry's own spawn tally. Two residual limits are accepted and stated, not closed: (1) a vehicle that greps the mutated lexer for the specific mutation signatures and simulates the expected effect would defeat behavioral mutation testing itself — it would be a second interpreter of the code under test and a deliberate, visible forgery in any review of the probe file, not a plausible refactor or accidental vacuity; (2) a deliberate forgery inside `dev/js-lexer.mjs` itself is outside the threat model by construction, not by choice: the mutation is applied to the file that then executes, so that file holds its own mutation as ordinary program state, and no differential can distinguish a lexer that executes its mutation from one that reads it — the definitional boundary of mutation testing. The lexer's integrity is a review obligation, not a gate claim. An accidental large-input shortcut in the lexer is inside what the differentials detect (measured against the round-50 mechanism: an early return above 1,000,000 units fails all eight differentials, and a scanner that stops charging inside identifier runs fails all eight). All six historically demonstrated facades are caught by the new gate. (Round-48 correction, inserted in place: what the re-verification ran against is not recorded; five of the six facades lived in `dev/validate-lexer-observations.mjs`, deleted by this same commit, so they are no longer reconstructible as written — read the sentence as an analytical argument from the differentials' mutually exclusive expectations, not as six documented fresh reconstructions. Round 48's review additionally demonstrated a seventh, ordinal-channel bypass of this gate — a vehicle answering from canned data without scanning — which the round-48 fixing disposition below closes by randomizing the differential execution order and noncing the error-valued expectations; the residual list in this paragraph was short by one.) (Round-49 correction, inserted in place: residual (1) itself was too generous — the bypass round 49 demonstrated against the round-48 disposition read no code text and simulated nothing: one `fs.statSync` on the temp-tree lexer plus a hardcoded baseline answered five of the eight differentials, control 401 included, and 39-/43-unit behavioral probes identified three more; the round-49 fixing disposition below closes both channels, and the residual becomes the per-run guessing probability stated there plus the unchanged second-interpreter forgery.) No broader closure of the bypass class than that is claimed, and no push, bump, tag, or publication is implied by this entry.

**Net tooling state.** The validator enforces exactly two anchored top-level trigger tables, a shared project-fence mask, a bounded code-span scanner, and explicit unsupported-style diagnostics; it structurally parses and deep-freezes `MODULES`/`AUDIT_UNITS`, including the pinned policy matrix and SHA-256-pinned coverage block. Its JavaScript mutation scanner charges every delimiter-stack step, rejects mismatched nesting, preserves private-name token roles, tracks class-body roles across brace-bearing `extends` expressions, and handles ordinary, static, private, computed, string, and numeric class-field names with a bounded lexical cache. Function/class-expression division, direct function-heritage ordering, class-field initializer roles, and the associated completion-loop/workflow-mutation boundaries are covered by controls 415–484; indirect provenance remains a runtime deep-freeze backstop. The Node.js 24 floor is guarded at startup, with CI definitions for current Node 24 and exact `24.0.0`. The fixture suite has 500 controls, with 425 child-process controls (396 validator-entrypoint and 29 focused lexer/helper children) and 75 in-process controls, a split the fixture registry now machine-checks against the spawns it actually routes. Controls 495–500 cover changelog release rollover. The 494-control/419-child/75-in-process split is historical to the pre-release spec-review state; the 492-control/416-child/76-in-process split is historical to the round-46 fixing state, the 490-control/414-child/76-in-process split is historical to the round-45 fixing state, the 484-control/409-child/75-in-process split is historical to the round-42 fixing state, and the 460-control/397-child/63-in-process split is historical to the round-42 review base at `633a0da`. Installer interruption/recovery cases live in the separate `dev/test-installer-recovery.mjs` matrix/helper and are not included in that numbered total. Focused lexer children emit a structured semantic observation that the parent judges against a behavioral differential — the same child run against a temp-tree copy of the repository whose `dev/js-lexer.mjs` carries a run-time-chosen mutation (or none) — the tested accidental shortcuts fail these expectations, while deliberate forgery by the vehicle or lexer remains outside the gate's integrity claim. Parser-only one-column, HTML-block, list-container, and `TABLE_VISITED` probes remain outside the anchored contract. The cycle history and round-23 disposition, including CI run `34015308368` (both jobs green at the reviewed 375-control head), are recorded in `docs/reviews/README.md` and the corresponding round-23 report; those counts and results are revision-qualified historical evidence, not current-head CI claims.
...
validated on Linux. (Round-48 correction, inserted in place: the whole mechanism this
paragraph and its P3-1 item describe — the scan-heap-ratio and retained-array work floors, their
telemetry, the 32 MiB RSS tripwire with the `RUST_INTEL_CONTROL401_MIN_PEAK_RSS_MB` override, and
control 458's source-level callee-identity pin — was retired by the anti-vacuity gate rebuild in
`6defc27` and exists in no current code path; the knob is read by no code, so setting it now does
nothing at all. This text is history, not current state.) P3-1: the "expected result is unknowable from source" claim is withdrawn as
...
This rejected the specific `as`-aliased module-scope wrapper facade the review demonstrated.
The length-gated fast-path vector inside `dev/js-lexer.mjs` itself is NOT closed by the work
floors: an allocating fast path satisfies the retained-array delta with a single
`new Uint8Array(source.length)` while never scanning. (Round-46 correction, inserted in place:
the original text here claimed the floors covered that vector structurally; that was false as
stated — see the round-46 fixing disposition below. The floors close non-allocating eliding
facades, which is the only reconstruction actually run; an allocating fast path inside
`dev/js-lexer.mjs` is excluded by the stated threat model — deliberate forgery of the lexer —
not by the floors.) Closure prose in this changelog, the ledger, and the code comments is narrowed to what
...
**Round-46 fixing disposition (implemented; ordinary Windows validation re-run locally).**
The round-46 review (`docs/reviews/latest-commits-review-round-46-2026-09-07-1826.md`) is disposed
as follows, committed as the commit containing this row, on top of `c2ee3fb` (the round-46 report
commit); push, bump, tag, and publication remain separate, explicit, human-authorized actions.
Closure below is stated per reconstruction actually run, not per class. P2-1: control 458's
callee-identity pin is reworked so both halves agree on what counts as source — the pinned import
must appear verbatim in the raw text AND its comment/string-masked image must appear as a live
line (a copy inside a comment or a string satisfies nothing; a forged module path fails the raw
half; the pinned image is computed by masking the pinned line itself, because the line's own
`'./js-lexer.mjs'` string is blanked by the same masking) — and the brittle exactly-twice
occurrence count is replaced by binding-shape assertions (no `function`/`const`/`let`/`var`/`class`
declaration of the identifier, no `as`-alias in either direction), evaluated by a shared
`evaluateObservationModuleContract` function used verbatim by the positive and negative controls.
Reconstructions run and rejected: (1) the review's comment-hidden-import wrapper facade
(block-commented pinned import + same-named wrapper retaining `new Uint8Array(source.length)` +
tail recovery), inserted into the real module — the full fixture suite exits 1 with control 458's
pin message naming the violated check (observed twice); (2) the round-45 `as`-alias wrapper
spelling — flagged by the binding-shape assertions. Residual stated honestly: a binding that is
neither a declaration nor an alias (for example a same-named forged parameter default) is not
asserted against; deliberate source forgery remains outside the threat model, as the control 401
comment already states. P2-2: control 458's `helperContract` now pins the work-floor telemetry's
provenance at source level — `initialMemory`/`scanMemory`/`companionMemory` sampled from
`process.memoryUsage()`, the scan-then-sample adjacency, the `scanHeapSample`/
`scanArrayBuffersSample`/`initialArrayBuffers` telemetry expressions, and the `peakHeapSource`
label — and `expectLexerProbe` re-asserts `peakHeapSource === 'sampled-around-scan-and-terminal'`
plus `peakRssSource` being `boundary-sample` or `process.resourceUsage.maxRSS` (the round-45
deleted telemetry-provenance assertion is restored and extended). Recorded distinction: removing
the sampling statements outright already fails control 401 closed at runtime (both work signals
evaluate to null, so the floor OR is false) — observed via control 491's diagnostic — so that
removal was a regression-clarity gap, not a live bypass; the pins additionally close the
replacement forgery (a fabricated `scanMemory` value no longer passes control 458's source
contract). P3-1: the false fast-path closure claim in the round-45 paragraph above is corrected
in place; the code comment at the floors and control 459's comment now state that the floors close
non-allocating eliding facades only, and that an allocating fast path inside `dev/js-lexer.mjs` is
out of the stated threat model rather than floor-closed. P3-2: control 487's mutated core phase now
writes a stdout sentinel and exits 7 — a value that is not the coordinator's fallback — and the
control asserts the coordinator's own status is 7, the `exit status 7` diagnostic, the sentinel's
arrival (the `stdio: 'inherit'` contract), `phase=core failed`, and the absence of both
`phase=core passed` and `phase=fixtures`. Discrimination observed: replacing the coordinator's
forwarding expression with a literal `process.exit(1)` makes control 487 the suite's only failure
(status 1 against the child's 7). P3-3: two negative controls close the gap for this round's own
mechanisms — control 491 removes the probe file's post-scan sampling in a temp copy and requires
the shared `evaluateWorkFloors` predicate to reject the resulting null telemetry while the
semantic oracle stays green (discrimination observed: neutering the shared predicate makes control
491 the only failure, with both samples null and the child still exiting 0); control 492 builds
both facade shapes from the real module source and requires the comment-hidden facade to pass the
child's full semantic oracle (which is why the parent-level pin must exist) while the shared
`evaluateObservationModuleContract` evaluation flags both facades and the unmutated module stays
clean (discrimination observed: neutering the shared evaluation's `identityHeld` makes control 492
the only failure). Control 492 also exposed a failure mode in this pass's first draft: a forged
module the lexer cannot scan crashed the suite with an unhandled `maskJsNonCode` exception; the
shared evaluation now returns a fail-closed `lexable: false` result and control 458 reports a
dedicated message. P3-4: `RUST_INTEL_CONTROL401_MIN_PEAK_RSS_MB` is documented in the README's
tooling paragraph beside the other knobs. (Round-48 correction, inserted in place: the knob was
then deleted outright by the `6defc27` gate rebuild — it is read by no code and setting it does
nothing — and this paragraph's P2-1/P2-2/P3-3 mechanisms (the `helperContract` pins, the telemetry
work floors, `evaluateObservationModuleContract`/`evaluateWorkFloors`, and the old controls
491/492 source-contract behavior) were retired with it; those control ids now belong to
lexer-mutation differentials.) P3-A: the semver paragraph above is re-anchored at
`v0.6.0`. Suite: the fixture suite is 492 controls — 416 child-process (390 validator-entrypoint
plus 26 focused) plus 76 in-process — with header, registry, README, and this changelog in
agreement. (Round-48 correction: "is" held only at that fixing state; the suite is now 494
controls — 419 child-process (390 validator-entrypoint plus 29 focused) plus 75 in-process —
under the behavioral-differential gate.) Fresh evidence for the open Windows gate: ordinary `npm run validate` at this fixing
pass's tree (Node v24.12.0, Windows 10.0.19045) passed with exit 0 and 492/492 controls in 432 s;
the recorded run predates only the insertion of its own figures into this sentence. This is
local non-reproduction evidence for the current fixing state, not a demonstrated fix; independent
HS review and exact-head CI remain release gates, and no version bump, tag, push, or publication
is claimed.

**Round-48 fixing disposition (implemented; ordinary Windows validation re-run locally).** The round-48 review (`docs/reviews/latest-commits-review-round-48-2026-09-08-0044.md`) is disposed as follows, committed as the commit containing this entry, on top of `a971bc5` (the round-48 report commit); push, bump, tag, and publication remain separate, explicit, human-authorized actions. P2-1: the anti-vacuity differentials no longer disclose their expected outcomes through the invocation ordinal. The eight differential controls are registered, executed, and completed as one group in a per-run randomized order (Fisher-Yates shuffle driven by `crypto.randomInt`), so the number of prior invocations of a probe argv no longer predicts which mutation — or none — a vehicle's temp tree carries; each control still registers, spawns, and completes immediately around its own child, preserving the registry's one-spawn-per-control attribution. Where the expected outcome is an error, the mutation now also embeds a run-time-random nonce in the message thrown by the mutated lexer itself — controls 402 and 458 join control 459's random K (402's mutate rewrites only the budget throw's message text; its condition, constant, and every other byte of `dev/js-lexer.mjs` are untouched, and the 401∧458 / 402∧491 operation-count pairings are unchanged) — so a canned static message cannot match an error-valued expectation even by luck. Success-valued expectations (401, 491, 492, 493, 494) cannot carry a nonce without changing their verified semantics and rely on the shuffle. (Round-49 correction, inserted in place: this sentence was false — the shuffle protected the success-valued expectations from nothing, because the nonce had made 402 and 458 length-changing and completed a `fs.statSync().size` fingerprint one stat could read, while sub-50-unit behavioral probes identified 492/493/494 independently; closed by the round-49 fixing disposition below.) (Round-50 correction, inserted in place: the shuffle, the padding, and the branch draws were retired outright by the round-50 fixing disposition below — every success-valued expectation itself carries a run-time scan-completion nonce now, which makes identification worthless and this paragraph's protection machinery dead weight.) In the same pass, every differential mutate asserts its anchors occur exactly once at run time (P3-5), and `expectLexerProbe`'s dead `expected` option is removed. Adversarial re-test, run for real: the review's reconstructed invocation-ordinal facade (a per-(ppid, argv) counter persisted in `os.tmpdir()`, canned answers for every (argv, ordinal) pair under the old fixed source order, a genuine call only where the answer was unknowable) fails the rebuilt suite — observed exit 1 with controls 402, 458, 459, 491, 492, 493, and 494 all failing (402 and 458 on the nonce their canned messages cannot carry, 459 on the unknown K, the rest because the shuffled order feeds each canned answer to the wrong control), control 401 being the only differential the canned table happened to satisfy that run. P3-1: the "re-verified by reconstruction" claim in the gate-rebuild paragraph above is corrected in place to what is actually recorded. P3-3: the superseded round-45 and round-46 paragraphs are annotated in place (the round-46 precedent), with `RUST_INTEL_CONTROL401_MIN_PEAK_RSS_MB` marked as read by no code; README's provenance sentence now qualifies that `dev/validate-lexer-observations.mjs` no longer exists. P3-4: the 64 MB heap-cap comment now describes the workloads that actually run under it, and the ragged control-485 comment is re-aligned with `dev/validate.mjs`'s precise historical wording. P3-A/P3-B: the review ledger's blank-line breaks are repaired so it renders as a table again, the rounds 43-47 series row is re-dated to its commit's real date and commit-anchored, and the missing round-47 and round-48 review rows are added. P3-C (coordinator option surface), P3-D (two unreferenced dev scripts), and P3-E (the primary checkout's untracked hooks directory) remain open, unchanged. Fresh evidence for the open Windows gate: ordinary `npm run validate` at this fixing pass's tree (Node v24.12.0, Windows 10.0.19045) passed with exit 0 and 494/494 controls in 328 s; the recorded run predates only the insertion of its own figures into this sentence. This is local non-reproduction evidence for the current fixing state, not a demonstrated fix; independent HS review and exact-head CI remain release gates, and no version bump, tag, push, or publication is claimed.

**Round-49 fixing disposition (implemented; ordinary Windows validation re-run locally).** The round-49 review (`docs/reviews/latest-commits-review-round-49-2026-09-08-0332.md`) is disposed as follows, committed as the commit containing this entry, on top of `4811697` (the round-49 report commit); push, bump, tag, and publication remain separate, explicit, human-authorized actions. P2-1 — the round-48 nonce made controls 402 and 458 length-changing, completing the `fs.statSync().size` fingerprint the previous round had measured and bounded (one stat identified the mutation within every probe argv, and the five success-valued differentials, control 401 included, were canned-answerable with zero scanning), plus an independent sub-50-unit behavioral-probe channel identifying 492/493/494 — is closed structurally rather than by pinning the vehicle: (a) every mutated tree in a run, the identity runs of 401 and 493 included, is padded after all executable code to one common byte length computed from that run's own draws (measured in the re-test below: all eight invocations reported the same size delta, +286), so the size channel has nothing to read; (b) success-valued expectations now derive from run-time draws observable only through the mutated text (the accepted second-interpreter residual) or a full-cost scan — control 491 draws a 50/50 accept/reject branch (budget raised by a random headroom; the argv-carried input length is drawn so its distribution is identical on both branches, so argv leaks nothing; reject = one operation above the raised budget on a nonce'd message), control 492 always renames the completion constant but draws whether the scan runs at the exact budget (accepted, no diagnostics) or one operation short (rejected, nonce'd message), and control 494 gates the block-comment branch behind a random operation threshold T ∈ [2, 999,957] (the decoy's comment-branch check measured at operation 999,958, so the decoy is always live while every sub-decoy probe stays masked exactly as in the unmutated scanner), making 493 and 494 mutually ambiguous below ~1,000,000 charged operations. Controls 401 and 493 are identity runs with no mutable answer; their protection is that size-and-behavior ambiguity plus the per-run shuffle. Residual restated honestly (superseding the residual-list annotations above): the three error-valued differentials force a full-length scan outright; the five success-valued differentials force a scan up to a per-run guessing probability — a facade that scans nothing and answers its argv's most likely canned shape passes with probability ≈ 1/64 per run (401: 1/4; 491/492/493/494: 1/2 each) — and a vehicle that reads the mutated temp-tree lexer and simulates it remains the accepted second-interpreter forgery. (Round-50 correction, inserted in place: the ≈ 1/64 guessing-probability residual is moot — the round-50 fixing disposition below deleted the mechanism that had that residual (the shuffle, the padding, the branch draws) rather than fixing it in place, and the five success-valued differentials now carry a fresh per-run scan-completion nonce, so a canned, eliminating, or fingerprinting facade constructs no success observation at all.) Adversarial re-tests, run for real against this fixing pass (each a scratch replacement of `dev/validate-lexer-probes.mjs`, reverted afterwards): the reconstructed round-48 invocation-ordinal facade failed the suite — exit 1 with differential controls 402, 458, 459, and 491 failing (static canned messages cannot match the nonces, and the shuffled order fed the canned 401/493/494 answers and the genuine-run 459 slot to the wrong trees), controls 401, 493, and 494 passing only by shuffle luck and 492 via its genuine-run fall-through; the reconstructed round-49 size/behavior facade failed the suite — exit 1 with differential controls 402, 458, 459, 491, 492, and 494 failing, after its own stderr showed every invocation of all three argvs measuring the identical size delta (+286) so no bucket matched and it scanned nothing, controls 401 and 493 passing only because their identity defaults happened to be right. (Both scratch facades replaced the whole vehicle file, so focused controls 399/400/409-478 also reported unsupported-argument failures — scaffolding noise of the re-test harness, not gate evidence. A first evidence run for this entry failed control 491 because that same scratch-restore procedure — `git checkout -- dev/validate-lexer-probes.mjs` — had also reverted the vehicle's uncommitted argv-length transport; the failure was diagnosed from the new P3-4 reproduction line, the vehicle re-applied, and the suite re-measured; recorded here because the restore hazard is part of this pass's evidence.) P3-1: the heap-cap comment now states the real bounds (controls 409-478 scan the whole `dev/validate-fixtures.mjs` source, ~266,000 code units and growing with every control added; 493/494 run 1,000,000-unit inputs). P3-2: README's provenance sentence now credits the gate rebuild to the round-47 fixing pass (`6defc27`). P3-3: the load-bearing punctuation of the differential scope headers is documented next to the headers — the 401/402 parentheticals keep them out of the label inventory carried by the `// Controls 400-402:` label, while the colon-bearing 458/459/491/492 headers ARE those controls' inventory entries. P3-4: any differential failure now prints the per-run execution order and every draw to stderr (observed live in both re-tests and in the failed first evidence run above), so a red run is diagnosable without a seed. P3-5, P3-C, P3-D, P3-E: open, unchanged. Fresh evidence: ordinary `npm run validate` at this fixing pass's tree (Node v24.12.0, Windows 10.0.19045) passed with exit 0 and 494/494 controls in 293 s; the recorded run predates only the insertion of its own figures and this pass's documentation edits into the tree. This is local non-reproduction evidence, not a demonstrated fix; independent HS review and exact-head CI remain release gates, and no version bump, tag, push, or publication is claimed.

**Round-50 fixing disposition (implemented; ordinary Windows validation re-run locally).** The round-50 review (`docs/reviews/latest-commits-review-round-50-2026-09-08-0721.md`) is disposed as follows, committed as the commit containing this entry, on top of `56add5f` (the round-50 report commit); push, bump, tag, and publication remain separate, explicit, human-authorized actions. A second design consultation rejected both of the review's proposed corrections before this pass was built: the variable-multiset P2-1 fix breaks the registry's one-spawn-per-control attribution and remains probabilistic rather than deterministic, and the move-entropy-into-the-input P2-2 fix was empirically disproved (built and measured: a forged vehicle answered the entropy-carrying input via `String.prototype.indexOf` in 0.2 ms against the real scan's 189 ms, so entropy embedded in the input does not force real work). P2-1 (elimination) is closed by making identification worthless instead of expensive: every success-valued differential's mutated `dev/js-lexer.mjs` now records a fresh per-run 48-bit nonce in a module binding when its scan's operation counter reaches that differential's measured final charged operation (measured against this tree: 2,000,000 for the 401/492 input and tree, 2,000,001 for 491's, 999,998 for 493's identity-plus-nonce tree — the `/*` and `*/` delimiters each consume two code units for one charged operation — and 1,000,000 for 494's fixed comment-disabled tree), and completionDiagnostics' return appends one trailing diagnostic entry `{ index: <input length>, id: <nonce> }` only when the nonce fired and the completion pass ran at that differential's full input length; the expectation is the former diagnostics plus that entry. A vehicle that identifies its tree — by order, elimination within a probe argv group, `os.tmpdir()` memory, file size, or a cheap behavioral probe — still cannot construct the answer, so the round-48 Fisher-Yates shuffle, the round-49 length-equalization padding, and the round-49 491/492/494 branch draws are deleted as dead weight rather than kept alongside: 491 and 492 revert to their pre-round-49 always-accept shapes (budget raised by exactly one to a fixed 2,000,001 at a fixed 2,000,001 input length; rename-only), 494 reverts to the fixed `false &&` comment-branch disable, 402/458/459 are byte-for-byte unchanged, and controls 401 and 493 stop being identity runs — each now carries only the nonce machinery. Deleting the padding pass also deletes its two top-level throws, so an anchor drift again fails the eight controls closed instead of aborting the fixture process (round-50 P3-4's mechanism is gone and the designed per-control message is reachable again), and the failure reproduction line now prints the eight per-run draws that still exist (three message nonces, K, five scan-completion nonces). Adversarial re-test, run for real against this fixing pass: round 50's elimination facade (a scratch `dev/validate-lexer-probes.mjs` persisting per-(ppid, argv-group) differential identity in `os.tmpdir()` and answering the last unidentified member of a probe argv group from its formerly-public canned answer without scanning) fails the suite — exit 1 with controls 491, 492, and 494 failing on the missing nonce entries, the fixed order making exactly those the last identified members of their groups, while every invocation the facade scanned honestly passed. P2-2 (the forged-lexer exclusion) is closed as a claim-narrowing, not a mechanism change: the gate-rebuild paragraph above and README's tooling paragraph now state the boundary the review asked for — the differentials prove the fixture's evidence is produced by executing the mutated scan through its final charged operation, a `dev/js-lexer.mjs` deliberately written to read its own mutation is outside the threat model by construction (the definitional boundary of mutation testing, for any code under test in any language), and the lexer's integrity is a review obligation. The two accidental-defect detection claims in that new wording are measured against this mechanism, not hand-traced: scratch copies of `dev/js-lexer.mjs` with (a) an early return above one million code units and (b) an identifier-continue loop that stops charging the operation counter were run against all eight differentials and each failed every one — 402/458/459 on the missing budget throw, the five success-valued controls on the missing nonce entry. P3-1, P3-2, P3-3, and P3-5, and the carried P2-A, P2-B, P3-C, P3-D, and P3-E, remain open, unchanged. Fresh evidence: ordinary `npm run validate` at this fixing pass's tree (Node v24.12.0, Windows 10.0.19045) passed with exit 0 and 494/494 controls in 496 s; the recorded run predates only the insertion of its own figures and this pass's documentation edits into the tree. This is local non-reproduction evidence, not a demonstrated fix; independent HS review and exact-head CI remain release gates, and no version bump, tag, push, or publication is claimed.
