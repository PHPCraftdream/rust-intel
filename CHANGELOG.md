...
**The post-`44eb16c` HS review-cycle 5 records an architectural correction to validator scope.** The validator intentionally validates the two anchored top-level trigger tables and a documented repository Markdown subset; it no longer claims to parse arbitrary GFM containers or inline links. Unsupported raw HTML, same-line container-prefixed fences, and complex inline markup in code-pattern cells fail explicitly; the bounded linear code-span duplicate scan remains. No new category, no BANNED/REQUIRED shape change — **PATCH**-shaped. This is a closure-scope record; it does not assert test results or a new commit.
