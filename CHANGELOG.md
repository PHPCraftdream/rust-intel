...
**The post-`11dc3df` independent Sol-high review cycle 11 closes the round-22 follow-up.** `AUDIT_UNITS` coverage now parses the complete workflow array, including indented headings; rule-text oracles are fence-aware; nested fixture recursion is skipped only by the exact `RUST_INTEL_SKIP_NESTED_FIXTURES` flag; and NBSP/tab-indentation controls use corrected diagnostics. The fixture watchdog defaults to 15 minutes and is overrideable with the positive-integer `RUST_INTEL_FIXTURE_WATCHDOG_MS` environment variable. Local verification passed: `npm run validate` reports 12 skill Markdown files and 113 hand-written controls; `node --check` passed for `dev/validate.mjs`, `dev/validate-fixtures.mjs`, and `bin/install.js`; and `node bin/install.js --help` passed. CI is pending because this correction has not been pushed, so no CI result is claimed. Provenance erratum: `11dc3df` contains literal `\n\n` text in its commit body, while `0d6facb` is subject-only; the correction commit uses a real body. No new category, no BANNED/REQUIRED shape change — **PATCH**-shaped.
