...
**The post-cycle-18 independent Sol-high review cycle 19 tightens the workflow contract.** `MODULES` and `AUDIT_UNITS` remain direct, deep-frozen declarative initializers and may be consumed inline; primitive `.length` bindings remain allowed. Direct RHS-leading reference aliases, destructuring of either root array or its nested arrays, and bindings of `.map()`/`.filter()` results are unsupported and rejected because bounded static scope analysis cannot prove that descendants are detached from the frozen graph. Workflow consumers are rewritten to explicit loops that project only primitive values, with fixed `.length` oracles and corrected comment ranges. Cycle 18's 217-control evidence remains historical. The current fixture count, tests, CI result, and active Codex-install parity are pending and are not claimed by this entry. No new category, no BANNED/REQUIRED shape change — **PATCH**-shaped.
