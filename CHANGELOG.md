...
- **§F1 persisted-format oracle rebuilt.** The golden-bytes corpus was typed-value-level (same wire bytes under different Rust types — bincode 2 `bool`/`u8`, spec-verified) and its recursive coverage unbounded; restated as a decode-observable permutation oracle (schema-mutation negative control as the mechanical form), over the finite graph of distinct serialized type/variant definitions with exactly two representatives per recursive edge, layered with the version-byte/dispatch/policy control required only when more than one layout is deployed.
