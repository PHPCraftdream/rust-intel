...
**The post-`44eb16c` HS review-cycle 7 records the maintainer-facing validator contract:** exactly two global top-level trigger-table anchors with canonical end/scaffold boundaries; raw column-1 pipes, declared widths, and nonempty bodies; continuation through the `Category map` parity check; duplicate keys defined as the sorted set of inline-code tokens, with prose-only rows excluded from deduplication; ordinary emphasis/strong/strikethrough allowed, while bracketed link/image/reference-like syntax, angle-leading raw-HTML/autolink-like syntax, and the broad repository-style URI/email-like ban outside inline code are rejected; standalone project fences with 0–3 leading spaces; and no claim to be an exact GFM parser. No separate review report was created; this row records the closure scope without asserting test results or a new commit.
