...
**The post-cycle-27 independent Sol-high review cycle 28 is integrated.** Scope: preserve the runtime-only escaped-spelling boundary for dotted `IdentifierName` access and escape sequences inside quoted bracket keys; add causal controls for valid braced escapes; cover nested-brace switch labels; and preserve the astral postfix identifier boundary. Cycle 27's 313-control evidence remains historical and is not silently rewritten. Verification completed: `npm run validate` passed (the main validator printed 12 skill Markdown files); direct `node dev/validate-fixtures.mjs` passed its 2 cases (the source header declares 325 controls); and separate `node --check` passed for the validator and fixture runner. CI and active Codex-install parity remain pending and are not claimed. No new category, no BANNED/REQUIRED shape change — **PATCH**-shaped.
