...
**The post-`44eb16c` HS review-cycle 6 records the maintainer-facing validator contract:** exactly two global top-level trigger-table anchors with their end/scaffold boundaries; raw column-1 pipe, width, and body-row rules; duplicate scanning limited to the allowed plain-text + inline-code subset with complex inline/extended-autolink syntax rejected; standalone project fences with 0–3 spaces; explicit rejection of container-prefixed fences and angle-leading raw-HTML-style lines; and no claim to be a general CommonMark/GFM parser. No new category, no BANNED/REQUIRED shape change — **PATCH**-shaped. This is a closure-scope record; it does not assert test results or a new commit.
