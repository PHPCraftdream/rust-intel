...
**Cycle 30's post-cycle-29 independent Sol-high follow-up is superseded by cycle 31.** Historical scope: distinguish a decimal/consequent ternary expression from optional chaining at the parser boundary, and add causal controls proving that each supported and rejected spelling reaches the relevant mutation path. Cycle 29's 339-control evidence and cycle 30's 343-control evidence remain historical and are not silently rewritten. Current tests and fixture count are pending; CI and active Codex-install parity remain pending and are not claimed. No new category, no BANNED/REQUIRED shape change — **PATCH**-shaped.

**Cycle 31 integrates the next independent Sol-high follow-up.** Scope: add a causal optional-chain label oracle and require `ternaryDepth === 0` at the relevant mutation boundary. Verification completed: `npm run validate` passed (the main validator printed 12 skill Markdown files); direct `node dev/validate-fixtures.mjs` passed its 2 cases (the source header declares 343 controls, unchanged); and separate `node --check` passed for the validator and fixture runner. CI and active Codex-install parity remain pending and are not claimed. No new category, no BANNED/REQUIRED shape change — **PATCH**-shaped.
