...
**Eight further review rounds (rounds 8–15) closed defects found by independent reviews of the batch above; normative fix commits `a0197be` (rounds 8–9), `f2b4897` (10), `31a3505` (11), `5417d3c` (12), `d10427d` (13), `adee2e9` (14), plus the round-15 synthesis pass (`docs/reviews/round-15-synthesis-2026-09-03-2146.md`).** No new category, no BANNED/REQUIRED shape change — **PATCH**-shaped. Full evidence per round in `docs/reviews/latest-commits-review-round-{8..15}-*.md`; this entry summarizes by group.
...
- **Tooling.** Leading-pipe convention enforced by a three-state header/delimiter/body table machine (header/delimiter rows losing their pipe are caught and cited by their own line); rule-text presence controls scoped per row; the fixture suite rewritten to a temp-copy allowlist with a junction/symlink physical-path guard (`fs.realpathSync.native`) and optional `.app.json`/`.mcp.json` inputs; Node floor 16.7.0 (`fs.cpSync`). Round 15 adds the `\"`-escape guard, corrects the GFM block-start allowlist (link reference definitions are not block starts; indented code blocks are), fixes a pipe-less-header state-machine gap, and pins the §B12 recipe, §B1a witness, and §B13 exemption with row-scoped controls. Round 18 narrows the `\"`-escape guard's fence closer to GFM's exact suffix class — ECMAScript `\s*` admitted form feed, vertical tab, or non-breaking space after a closing delimiter, so a form-feed-suffixed line wrongly closed a fence and flagged a legitimate in-code `\"` as a leaked prose escape; the closer now accepts ASCII `[ \t]*` only (all shipped markdown is `\n`-ended), and the fixture suite gains the missing tab-indented fake-closer control plus a form-feed-suffix false-closer control (`docs/reviews/latest-commits-review-round-18-2026-09-04-0754.md`, both P3 findings).
