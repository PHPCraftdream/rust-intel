...
- **Tooling.** Leading-pipe convention enforced by a stateful header/delimiter/body/rejected table machine within the documented trigger-table and repository-Markdown subset (header/delimiter rows losing their pipe are caught and cited by their own line); rule-text presence controls scoped per row; the fixture suite rewritten to a temp-copy allowlist with a junction/symlink physical-path guard (`fs.realpathSync.native`) and optional `.app.json`/`.mcp.json` inputs; Node floor 16.7.0 (`fs.cpSync`). Round 15 adds the `\"`-escape guard, corrects the GFM block-start allowlist (link reference definitions are not block starts; indented code blocks are), fixes a pipe-less-header state-machine gap, and pins the §B12 recipe, §B1a witness, and §B13 exemption with row-scoped controls. Round 16 (tooling half, `91077f4`) moves the `\"`-escape guard's fence-state tracking from a blind boolean toggle to tracking the opener's marker character and length (so it no longer mis-closes a longer fence on a shorter same-character delimiter line), and gives `blockStartRe` end-of-line-terminated ATX-heading/list-item markers plus column-based (not literal-prefix) tab-expansion for indented-code detection — five new fixture controls. Round 17 (`e76c372`) narrows fence opener/closer indentation from any-whitespace to literal spaces only (0-3), rejects a backtick-fence opener whose info string itself contains a backtick, adds three new fixture controls, and corrects the correction-ledger sentence distinguishing the unchanged calibration fixture from the genuinely remapped record. Round 18 narrows the `\"`-escape guard's fence closer to cmark-gfm's exact closer-suffix class — ECMAScript `\s*` admitted form feed, vertical tab, or non-breaking space after a closing delimiter, so a form-feed-suffixed line wrongly closed a fence and flagged a legitimate in-code `\"` as a leaked prose escape; the closer now accepts ASCII `[ \t]*` only (all shipped markdown is `\n`-ended), and the fixture suite gains the missing tab-indented fake-closer control plus a form-feed-suffix false-closer control (`docs/reviews/latest-commits-review-round-18-2026-09-04-0754.md`, both P3 findings). Round 19 narrows `blockStartRe` and the table machine's blank-line test from ECMAScript `\s`/`trim()` to literal space/tab (a non-breaking-space-prefixed bullet or NBSP-only line no longer silently ends a table without a leading-pipe error), adds two fixture controls, corrects the fixture header's stale control count, re-cites the fence-closer suffix class from "GFM §4.5" to cmark-gfm's scanner (with the `.gitattributes` `eol=lf` dependency now stated), and lands two normative precision fixes (the Scope paragraph's associated-const dyn-compatibility clause and §C10's rustls `0.24` sentence restated as pre-release) (`docs/reviews/latest-commits-review-round-19-2026-09-04-0949.md`, six P3 findings).
...
**The post-`44eb16c` HS review-cycle 4 closure is superseded by cycle 5's P3 corrections.** Its preliminary scope covered accurate list-container lifecycle (siblings, residual markers, blanks, interruption, and padding); 0–3-space HTML starts; inline link/image destinations and titles; exact inline HTML priority; autolink bounds; and strengthened table-leak/parity oracles. No new category, no BANNED/REQUIRED shape change — **PATCH**-shaped. This records the supersession and does not assert test results or a new commit.

**The post-`44eb16c` HS review-cycle 5 closure is superseded by cycle 6's contract correction.** Its preliminary scope described an architectural correction to validator scope: the validator intentionally validates the two anchored top-level trigger tables and a documented repository Markdown subset; it does not claim to parse arbitrary GFM containers or inline links. Unsupported raw HTML, same-line container-prefixed fences, and complex inline markup in code-pattern cells fail explicitly; the bounded linear code-span duplicate scan remains. No new category, no BANNED/REQUIRED shape change — **PATCH**-shaped. This records the supersession and does not assert test results or a new commit.

**The post-`44eb16c` HS review-cycle 6 closure is superseded by cycle 7's contract precision.** Its preliminary scope covered exactly two global top-level trigger-table anchors, raw column-1 pipe/width/body rules, a bounded plain-text + inline-code subset, standalone project fences, and explicit rejection of unsupported container/angle-leading syntax. No new category, no BANNED/REQUIRED shape change — **PATCH**-shaped. This records the supersession and does not assert test results or a new commit.

**The post-`44eb16c` HS review-cycle 7 closure is superseded by cycle 8's contract precision.** Its preliminary maintainer-facing validator contract covered exactly two global top-level trigger-table anchors with canonical end/scaffold boundaries; raw column-1 pipes, declared widths, and nonempty bodies; continuation through the `Category map` parity check; duplicate keys defined as the sorted set of inline-code tokens, with prose-only rows excluded from deduplication; ordinary emphasis/strong/strikethrough allowed, while bracketed link/image/reference-like syntax, angle-leading raw-HTML/autolink-like syntax, and the broad repository-style URI/email-like ban outside inline code are rejected; standalone project fences with 0–3 leading spaces; and no claim to be an exact GFM parser. No separate review report was created; this records the supersession without asserting test results or a new commit.

**The post-`44eb16c` HS review-cycle 8 closure is superseded by cycle 9's P3 corrections.** Its preliminary scope covered fence-aware category parity; executable workflow parity; bounded child processes; one fixture aggregation path; the supported Node 16.7.0 floor; and the maintainer-facing contract that any raw `<` outside inline code in a code-pattern row's first cell is unsupported. No new category, no BANNED/REQUIRED shape change — **PATCH**-shaped. This records the supersession and does not assert test results or a new commit.

**The post-`44eb16c` HS review-cycle 9 closure covers:** code-state-only `MODULES` assignment; unique category owners and bodies; raw `<` outside inline-code spans; and the corrected linear stress oracle. No new category, no BANNED/REQUIRED shape change — **PATCH**-shaped. No separate review report was created; this row records the closure scope without asserting test results or a new commit.
