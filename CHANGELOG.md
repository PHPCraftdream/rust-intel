...
**Net validator state after independent Sol-high review cycle 10:** the gate checks exactly two anchored top-level trigger tables, a `projectFenceOpener`-based fence mask, bounded code-span scanning, explicit unsupported-style diagnostics, code-state-only `MODULES` parsing, and unique category ownership/body checks. Carried controls cover the four-backtick marker-length case, tab-indented fake opener/closer, form-feed closer suffix, NBSP body/delimiter cases, CRLF/lone-CR normalization, the invalid backtick-info body row, and raw-column-1 whitespace; parser-only one-column, HTML-block, list-container, and `TABLE_VISITED` controls are retired as moot under the anchored scope. The NBSP arbitrary-table counterexample from rounds 20–21 is likewise moot. See `docs/reviews/latest-commits-review-round-22-2026-09-05-2036.md`. Evidence recorded here includes root's earlier successful `npm run validate` after cycle 9 with 86 controls. The current restored-suite `npm run validate` is PASS with 97 controls in its header/count, and the main validator reports 12 skill Markdown files checked. No new category, no BANNED/REQUIRED shape change — **PATCH**-shaped.
