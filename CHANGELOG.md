...
**The post-cycle-17 independent Sol-high review cycle 18 narrows and hardens the workflow contract.** `MODULES` and `AUDIT_UNITS` remain direct, deep-frozen declarative initializers and may be consumed inline. Direct RHS-leading reference aliases or destructuring of either root array or its nested arrays are unsupported and rejected because bounded static scope analysis cannot prove complete mutation provenance. Primitive `.length` bindings and derived `.map()`/`.filter()` result bindings are allowed because they do not alias the frozen graph; direct `.length` mutation coverage and calibrated alias controls close the corresponding review paths. Cycle 17's 192-control evidence remains historical. Verification completed locally: `npm run validate` passed (main validator printed 12 skill Markdown files); direct `node dev/validate-fixtures.mjs` passed (2 cases; source header declares 217 sequential controls); separate `node --check` checks for the validator, fixture runner, and canonical/mirror workflow files passed; and `node dev/sync-mirror.mjs --check` passed for 13 files. CI and active Codex-install parity remain pending and are not claimed by this entry. No new category, no BANNED/REQUIRED shape change — **PATCH**-shaped.
