...
**§A1 gains the supply-chain lifecycle gaps a dedicated audit surfaced.** Three rule bullets + a documentation/grounding refresh, no new categories (still **58**), no tier change. Sourced from `docs/reviews/gap-audit-supply-chain-build.md` (each finding verified against the exact existing §A1 text — re-verified against current text after the five sibling releases shipped tonight; "yank", "`build.rs`"+network, "`[patch]`"/"`git =`"/"`rev`" in a dependency sense all appeared nowhere in §A1 before this release — and grounded in a citable primary source). The report's thesis: §A1 is dense on *which crate name* (slopsquatting, typosquat, dependency confusion, the malicious-dep build-time-execution vector, the `cargo-deny`/`cargo-vet`/`Cargo.lock` mitigation stack) but blind to what happens to a *trusted* dependency's bytes over time — yanked pins, unpinned build-time downloads, and moving `[patch]`/git sources that all walk around the audit stack §A1 already prescribes. Per the v0.4.3/v0.4.6/v0.4.7/v0.4.9/v0.4.10 sibling-bullet precedent (bullet/documentation-only additions, no new lettered category), this is a **PATCH** bump — none of the four findings became a new lettered sub-category; the report's own placement suggestions were all "new bullet in §A1" (findings 1–3) or "extend `docs/sources.md`" (finding 4), and forcing them there stayed coherent (all three rule bullets extend §A1's existing lockfile / build-time / dependency-trust framing).

### Changed

- **`skill/deps-macros-ergonomics.md` — §A1** gains three rule bullets and a proc-macro-trust citation (findings 1–3):
  - **Yanked-version handling under `--locked`** (🟡, →🔴 when the yank reason is an advisory the existing §A1 umbrella owns) — a new "Lockfile hygiene and yanked versions" paragraph *extending* (not replacing) the existing commit-`Cargo.lock`/`--locked` bullet: a yanked version stays downloadable and an existing lockfile keeps resolving it, so `cargo build --locked` against a yanked pin emits only a **warning** (exit 0, green forever in CI), while only a *fresh* resolution refuses it — a project can build for months against a version yanked for a security reason. REQUIRED: `cargo deny check advisories` (explicit yanked check) / `cargo audit -D warnings` turns the warning red with a stated reason. BANNED: the reflexive blanket `cargo update` (swaps many *other* pins — Tier-A residue); the fix is a targeted `cargo update -p <crate>`.
  - **Network access in your own `build.rs`** (🟡, →🔴 if the fetched artifact is executed at build time — that collapses into the existing 🔴) — a new bullet in the build-time paragraph, framed explicitly as the **write-direction twin** of the existing read-direction malicious-dependency rule (the device the v0.4.6 padding-leak bullet established): shelling out to `curl`/`git clone` or using `reqwest::blocking` in your own `build.rs` pulls *unpinned bytes into the build outside the lockfile* — `--locked`/`--offline`/`cargo vendor` give zero integrity (the download is not in Cargo's hash-verified graph), the artifact can change/be MITM'd between builds, reproducibility is gone, offline/vendored builds fail. REQUIRED: no network in `build.rs` — vendor the artifact, or fetch in a separate CI step with a hardcoded SHA-256 + offline fallback. Calibration: build-time *codegen* from a **local** schema/toolchain is fine — only network access is the finding.
  - **Unpinned `[patch]`/git overrides** (🔴 unpinned-HEAD/unnamed fork, 🟡 rev-pinned user-approved temporary patch) — a new BANNED bullet + a new REQUIRED bullet: `[patch.crates-io] tokio = { git = "…", branch = "…" }` with no `rev` follows remote HEAD, the source can change on any lockfile refresh, and `cargo audit`/`cargo vet`/RUSTSEC all key on *registry versions* so a git/`[patch]` override walks *around* the entire audit stack §A1 prescribes — framed as the **same class of unverified-trust decision** as adding an unknown dependency name, disguised as "it's still `tokio`, just my fork". REQUIRED: a full-commit `rev`, a user-approved repo, a post-flight flag (§A1's 🔴 protocol), and a stated removal condition. Calibration: a rev-pinned, user-approved, temporary patch with a removal condition is explicitly fine (🟡) — only the unpinned-HEAD/unnamed-fork case is BANNED; not all git dependencies.
  - **Incident base + proc-macro-trust citation** (finding 4) — §A1's "Real attack cases" list gains `evm-units`/`uniswap-utils` and `finch-rust`/`sha-rust` (both 2025-12), the 2026-02 crates.io notification-policy update, and the 2026-05 TrapDoor campaign (the takedown *cadence* is the signal); the proc-macro-trust sentence in the build-time paragraph — previously uncited — now cites the `serde_derive` precompiled-binary episode (2023, reverted v1.0.184 after community pushback) as the canonical demonstration that "it's a trusted crate" is not proof the code running at build time is the readable code.
- **`skill/SKILL.md`** — three phrase-trigger rows (use my fork / patch the dependency / the fix isn't released yet → §A1 unpinned `[patch]`/git; download in build.rs / fetch protoc/schema at build time → §A1 network in `build.rs`; fix the CI dependency error / candidate versions were yanked → §A1 yanked-crate handling) and two code-pattern rows (`[patch.*]` table or `git = "…"` with only `branch`/no `rev` → §A1; `build.rs` containing `curl`/`git`/`reqwest`/any network call → §A1).
- **`commands/rust-intel-cc/fix.md`** — three routing rows (CI passes today / fails next week with "candidate versions … were yanked" → §A1 yanked handling; reproducible build fails / different bytes on rebuild / `build.rs` shells out to `curl` → §A1 network-in-`build.rs`; `[patch]`/git dep changed behavior after a lockfile refresh, `cargo audit`/`vet` sees nothing → §A1 unpinned override).
- **`docs/sources.md`** — four new incident entries in "Documented incidents" (`evm-units`/`uniswap-utils` + `finch-rust`/`sha-rust` 2025-12; crates.io 2026-02 policy update; TrapDoor 2026-05; serde_derive precompiled binary 2023) and a new "Supply-chain lifecycle — §A1 gap additions (v0.4.11)" subsection: three grounding entries (Cargo Book `cargo yank` + cargo-audit Inside-Rust blog + cargo-deny advisories-config yanked check; Cargo Book Build Scripts + cargo#5720 + reproducible-builds.org; Cargo Book Overriding/git Dependencies + the `onering` account-compromise, Aikido 2026). Documented-mechanism; the incident anchor is the `onering` compromise.
- **`README.md`** — status block for v0.4.11.
- **`package.json` + `.claude-plugin/plugin.json`** — version 0.4.10 → 0.4.11.

## [0.4.10] — 2026-07-09

**§B14/§B3a/§B13/§B17 gain the concurrency & admission-control exhaustion gaps a dedicated audit surfaced.** Five findings (bullets/amendments), no new categories (still **58**), no tier change. Sourced from `docs/reviews/gap-audit-concurrency-exhaustion.md` (each finding verified against the exact existing §B14/§B3a/§B13/§B17/§B9/§E4 text — re-verified against current text after the four sibling releases shipped tonight — and grounded in a citable source). The report's thesis: both concurrency modules are dense on unbounded *data* growth (§B14) and *leaked* resources (§F3/§B21) but blind to unbounded *admission* — object counts, key cardinality, synchronized load — where each individual object is perfectly managed and the exhaustion lives one level up. The words "Semaphore", "EMFILE", "jitter" (as a requirement), "acquire_owned", "eviction", and "RwLock" (in §B17) appeared nowhere in the relevant bodies before this release. Per the v0.4.3/v0.4.6/v0.4.7/v0.4.9 sibling-bullet precedent (bullet/amendment-only additions, no new lettered category), this is a **PATCH** bump — none of the five findings became a new lettered sub-category; the report's own placement suggestions were all "bullet in an existing §" or "in-place amendment", and forcing them there stayed coherent (§B14's unbounded-admission and insert-only-cache bullets extend its existing "unbounded X" framing from queues to admission and cardinality; the RwLock reentrance is genuinely §B17's third reentrance flavor).

### Changed

- **`skill/concurrency-and-state.md` — §B14** gains three BANNED/REQUIRED bullets (findings 1, 2, 3):
  - **Unbounded task/connection admission** (🔴 peer-extendable, 🟡 otherwise) — an accept/ingest loop that spawns one task per item with no cap grows *nothing the existing channel/`Vec`/`FuturesUnordered` bullets name*: the unbounded resource is the task/socket/buffer/FD *count*, and the attacker sets it (CWE-770). A `// fire-and-forget: detached by design` comment satisfies §B21 and still admits unboundedly. REQUIRED: `Arc<Semaphore>` + `acquire_owned().await` **before** `tokio::spawn`, permit **moved into** the task. The two LLM-typical wrong fixes are named: acquiring the permit *inside* the task (tasks still unbounded, only the work is throttled) and binding it in the loop without moving it in (dropped before the task runs). Calibration: targets a public-facing/attacker-reachable accept loop, not an internal fan-out over a known-bounded worklist. Cross §B21/§F3.
  - **Accept-error handling** (🟡) — the sibling of the cap: `let (s,_) = listener.accept().await?;` propagates a single `EMFILE`/`ECONNABORTED` and kills the loop forever; the naive `Err(e) => { warn!(%e); continue; }` busy-spins at 100% CPU on `EMFILE` (listener stays ready, kernel keeps completing handshakes into the backlog). Explicitly **not** §B3a's case — an accept loop must not "release leadership and return" on a transient error and must not hot-retry. REQUIRED: `match` and classify — fatal (listener gone → stop) vs transient-with-backoff (`EMFILE`-class → brief `sleep`, then continue). tokio's listener docs carry the match-don't-`?` example.
  - **Insert-only keyed collection as an unbounded cache/registry** (🔴 attacker-extendable keys, 🟡 otherwise) — a cache *has* a consumer (constant reads), so the existing "`Vec` with no consumer" bullet does not match; the defect is *no eviction*, entries are immortal. Same shape as per-IP rate-limiter maps, session registries, dedup sets, high-cardinality metrics maps, the §B13 per-key `OnceCell` map. BANNED: insert-only keyed collection with unbounded/attacker-influenced key cardinality and no stated bound. REQUIRED: an LRU/TTL/cap via `lru`/`moka`, or a written argument that cardinality is internally bounded. CWE-770/400. Cross §B10/§B13.
- **`skill/concurrency-and-state.md` — §B13** gains a one-line warning appended to its endorsed per-key `Arc<OnceCell>` REQUIRED pattern (finding 3): that map is itself insert-only and grows forever under unbounded/attacker keys — bound it or state why the key space is closed (cross §B14).
- **`skill/concurrency-and-state.md` — §B17** gains a fourth BANNED bullet, the **third reentrance flavor** (finding 5, 🟡): RefCell reentrance panics, tokio Mutex reentrance deadlocks, and `RwLock` reentrant *reads* deadlock on a writer's schedule — `std::sync::RwLock`'s priority policy is unspecified/OS-dependent (std docs warn with exactly this example: read → helper re-reads → deadlocks behind a waiting writer on a writer-preferring OS; passes on dev, hangs on prod), and `tokio::sync::RwLock` is *documented write-preferring* so read → (task requests write) → same-task read **always** deadlocks. Distinguished from §B9 (that is two-lock ABBA; this needs one lock) and §B2 (no `.await` for the std flavor). REQUIRED: document read-reentrance-freedom the way §B17 already documents borrow-disjointness; drop the guard before the reentrant call or pass the data down. Bans undisciplined reentrant reads through a hidden call graph, not `RwLock` itself.
- **`skill/concurrency-and-state.md` — §E4** gains a cross-ref clause on its existing `RwLock` line pointing at the §B17 reentrant-read deadlock (finding 5).
- **`skill/async.md` — §B3a** REQUIRED backoff clause amended **in place** (finding 4 — the one finding in the *other* module): "back off exponentially (`tokio::time::sleep`) **with jitter**, never a tight immediate loop" — deterministic `BASE * 2^attempt` satisfies "back off exponentially" verbatim yet synchronizes N instances into lockstep thundering-herd waves that re-knock the recovering dependency over; add full/decorrelated jitter and a retry budget when the caller fans out. Not a new category — squarely inside §B3a's coordinator/retry charter, adding the jitter requirement it lacked.
- **`skill/SKILL.md`** — three phrase-trigger rows (accept loop / handle each connection / spawn per request → §B14+§B21+§F3; cache per user/IP/session / remember which X we've seen / dedup set → §B14; "reconnect on disconnect" added to the existing retry/backoff row, now routing to §B3a for jitter) and three code-pattern rows (uncapped `accept()`+`tokio::spawn` loop → §B14; `map.insert(key,v)` with request-derived key and no eviction → §B14; second `RwLock::read()` reachable while a first read guard is live → §B17).
- **`commands/rust-intel-cc/fix.md`** — six routing rows (server OOMs/FD-exhausts under many clients → §B14 admission; accept loop dies on `EMFILE` or spins after `warn!;continue` → §B14 error classification; per-user/IP cache never evicted → §B14; one-lock `RwLock` hang reproducing on prod OS only → §B17; every instance retries in lockstep after an outage → §B3a jitter; plus the §B10/§B14 split on the existing steady-RAM-growth row).
- **`docs/sources.md`** — new "Concurrency & admission-control exhaustion — §B14/§B3a/§B13/§B17 gap additions" subsection: five grounding entries (tokio `Semaphore` docs + discussions #5091/#2648 + CWE-770; tokio `TcpListener`/`UnixListener` accept-error match example + `accept(2)`; CWE-770/400 + `lru`/`moka` as mainstream mitigations, with CVE-2026-33012/CVE-2026-41310 cited *honestly* as cross-language pattern grounding, not Rust incidents; AWS "Exponential Backoff And Jitter" + Builders' Library "Timeouts, retries, and backoff with jitter"; std `RwLock` docs + tokio `RwLock` docs + rust-lang/rfcs#2674). Documented-mechanism + cross-language-pattern; no numeric claims.
- **`README.md`** — status block for v0.4.10.
- **`package.json` + `.claude-plugin/plugin.json`** — version 0.4.9 → 0.4.10.

## [0.4.9] — 2026-07-09

**§B7/§B20 gain the deserialization & parsing DoS gaps a dedicated audit surfaced.** Five findings (bullets/sub-clauses), no new categories (still **58**), no tier change. Sourced from `docs/reviews/gap-audit-deserialization-dos.md` (each finding verified against the exact existing §B7/§B20/§B26 text — re-verified against current text after two sibling releases shipped tonight — and grounded in a citable source: flate2/zip/rawzip docs, serde issues #3023/#2186/#2363/#1495/#2035, RUSTSEC-2018-0005, `deny_unknown_fields` container-attr docs, RFC 8259 §4, CWE-409/190/789). The words "decompress", "gzip", "flate2", "IgnoredAny", "serde_stacker" appeared nowhere in the skill before this release. Per the v0.4.3/v0.4.6/v0.4.7 sibling-bullet precedent (bullet-only additions, no new lettered category), this is a **PATCH** bump — none of the five findings became a new lettered sub-category; the report's own placement suggestions were all "bullet in an existing §" or "sub-clause of an existing bullet," and forcing them together stayed coherent (§B7's decompression bomb and parser-recursion sub-clause are both allocation/recursion-shaped DoS bullets in the same category).

### Changed

- **`skill/unsafe-and-ffi.md` — §B7** gains two BANNED bullets and one sub-clause:
  - **Decompression bomb** (🔴) — a new BANNED bullet directly after the `with_capacity(untrusted_n)` bullet: `decoder.read_to_end(&mut out)` on a gzip/zlib/deflate/zip/tar body with the *compressed* input capped but the *decompressed* output unbounded amplifies ~1 KiB → multi-GiB → OOM, with no attacker-controlled integer literal anywhere. A genuinely distinct shape from `with_capacity(n)` — no clampable `n`, amplification inside a trusted-looking decode call — so its own bullet, same category. Capping the compressed side (a `Content-Length` check, or the `zip` crate's own internal `.take(compressed_size)`) is *active false safety*; `flate2` has no output limit. REQUIRED: `.take(MAX+1)` on the **decoder**'s output (the mirror of the existing `Read::take` advice, applied to the decoder's output not the socket's input). CWE-409.
  - **Parser-recursion sub-clause** — extends the existing "no depth limit" recursion BANNED bullet (does not duplicate it): the depth limit inside a third-party parser applies to its *parse-from-text* phase only — serde_json's default 128 guards `from_str`/`from_slice`, but re-deserializing an already-built `serde_json::Value` (`from_value`, `IgnoredAny`, `flatten`/`untagged` via the buffered `Content` tree) walks the AST with no depth check → uncatchable stack overflow; serde_yaml has no limit at all (RUSTSEC-2018-0005). You cannot add a `depth` param to `IgnoredAny`/a derived `Deserialize`; mitigate with `serde_stacker` or cap input depth/size before the AST is built.
  - **`checked_mul` cross-ref** — a one-line BANNED bullet: a wrapping `count * item_size` (or `w * h * bpp`) in release silently *passes* a size clamp, then the original huge `count` OOMs (CWE-190→789) — the overflow defeats the very bounds check meant to stop this. The intersection of §B26 (overflow) and §B7 (allocation); cross-referenced from §B26. **Not** a new item — Gap 5 in the report was self-flagged as marginal; it was implemented as the requested one-line cross-ref (with a §B26 mirror pointer) because the "overflow neutralizes a bounds check that looks correct" angle is spelled out nowhere in the existing §B26/§B7 text.
- **`skill/data-and-types.md` — §B20** extends the existing `flatten` bullet and adds two REQUIRED bullets:
  - **Flatten buffer-everything cliff** — a new sentence on the existing `flatten`+`deny_unknown_fields` BANNED bullet (§B20 already owns `flatten`): adding `flatten`/an internally-tagged enum silently switches serde from streaming into "buffer the entire input into a `Content` tree, then re-deserialize" — the whole body is materialized regardless of fields kept (§E2 cross-ref), ~2× slower (serde #2363 measures ~50–86%), errors lose line/column and field names. Explicit calibration: `flatten` is a legitimate feature — the finding is shipping it un-anticipated on a hot/untrusted path, not the attribute.
  - **`deny_unknown_fields` on untrusted request structs** — REQUIRED where unexpected-field rejection matters (auth/authorization/financial/validation): default serde *ignores* unknown fields (mass-assignment / typo-smuggling — `{"amount":5,"amout":5000}`; OWASP API8:2023). Explicitly the *opposite* direction from the existing BANNED note (which warns the attribute does not catch typos in your own serialize names); both stated as true and non-contradictory. Incompatible with `flatten`. Calibration: not universal — do not add it to a forward-compatible permissive API.
  - **JSON duplicate-key last-wins** — REQUIRED awareness bullet: JSON object duplicate keys are last-wins by default (RFC 8259 §4), a parser-differential request-smuggling vector against an upstream proxy/WAF reading the first value.
- **`skill/data-and-types.md` — §B26** gains a one-sentence pointer on the bare-arithmetic BANNED bullet to the `count * item_size`-defeats-clamp site (cross-ref §B7).
- **`skill/SKILL.md`** — four phrase-trigger rows (decompress/gunzip/inflate/unzip → §B7 decompression bomb; re-deserialize a Value/from_value/middleware JSON/parse YAML from upload → §B7 recursion sub-clause; validate request body/reject unexpected fields/PATCH-PUT body/mass assignment → §B20) and four code-pattern rows (`GzDecoder`/`ZipArchive` + `read_to_end`/`extract` with no output cap → §B7; `serde_json::from_value`/`IgnoredAny`/`serde_yaml::from_*` on untrusted input → §B7; `count * size_of` clamped-then-allocated without `checked_mul` → §B7/§B26; `#[derive(Deserialize)]` untrusted-body struct without `deny_unknown_fields` / `flatten` on a hot path → §B20).
- **`commands/rust-intel-cc/fix.md`** — five routing rows (OOM decompressing a small capped body → §B7; stack overflow re-deserializing a `Value`/deep YAML → §B7; length clamp passes but allocation OOMs → §B7/§B26; client smuggles extra/duplicate JSON field → §B20; deserialization ~2× slower after `flatten` → §B20).
- **`docs/sources.md`** — new "Deserialization & parsing DoS — §B7/§B20/§B26 gap additions" subsection: five grounding entries (flate2/zip/rawzip no-output-limit + CWE-409; serde #3023 AST-bypass + serde_json Deserializer + serde_stacker + RUSTSEC-2018-0005; flatten buffering across serde #2186/#2363/#1183/#2035/#1495 + Ronacher; `deny_unknown_fields` container-attr + RFC 8259 §4 + OWASP API8:2023; CWE-190→789 for the overflow-defeats-clamp pattern). Documented-mechanism; the one numeric anchor is serde #2363's measured ~50–86% regression.
- **`README.md`** — status block for v0.4.9.
- **`package.json` + `.claude-plugin/plugin.json`** — version 0.4.8 → 0.4.9.

## [0.4.8] — 2026-07-09

**FFI/unsafe boundary gains the edge cases a dedicated audit surfaced, plus a new lettered §B25a.** Five findings across §B5/§B25, plus one new lettered sub-category **§B25a** ("the C library's own concurrency contract"). Numbered category count is unchanged (still **58**) — §B25a is counted under §B25, exactly as §B18a is counted under §B18 per the standing convention ("the count is of numbered categories; lettered sub-sections are counted under their parent"). Sourced from `docs/reviews/gap-audit-ffi-unsafe.md` (each finding verified against the exact existing §B5/§B18/§B25 text and grounded in a citable source: Rust Reference UB list + Unions page, Nomicon FFI chapter, Rust 2024 `unsafe(no_mangle)` guide, RUSTSEC-2020-0071/0159). The word "union" appeared nowhere in the skill before this release. This is a **PATCH** bump, per direct precedent: §B18a (and §C1a) were introduced in v0.3.2 — also a new lettered sub-category, also "counted under parent, count unchanged" — and that shipped PATCH; the immediately-preceding v0.4.7 release made the same call explicitly, declining a §B24a specifically "to avoid a 58→59 count churn" and shipping PATCH. A new lettered sub-category that leaves the numbered count untouched is this repo's established PATCH-shaped event, not a MINOR one — MINOR is reserved for a numbered-count change.

**Why §B25a is its own section (the §B18a split test).** §B18a was split from §B18 because "there is no `unsafe` token at the site that decides variance" — a genuinely separate failure locus. §B25a passes the same test: §B25 is about the correctness of the boundary code *you* write (panics you emit, ABI of types you pass, ownership of pointers you mint); §B25a is about a contract of code you did **not** write and cannot see — whether the C library is thread-safe/reentrant and what it demands of `init`/globals. No correctness in the wrapper's own data closes it, and the reflexive §B18 per-handle `Arc<Mutex<_>>` fix *actively masks* it: two independently-locked handles still race on the library's process-global state. Folding "audit the foreign code's concurrency contract" into §B25's panic/ABI/ownership prose would mix "what you must do" with "what you must learn about someone else's code" and make it incoherent — so it is its own 🔴 section.

### Changed

- **`skill/unsafe-and-ffi.md` — new §B25a "The C library's own concurrency contract"** (🔴). BANNED: `unsafe impl Send`/`Sync` for an FFI handle (or spreading calls across threads) without citing the library's documented thread-safety level; giving a non-reentrant library per-handle locks (the §B18 fix misapplied — per-handle locks don't serialize the library's globals); reading `last_os_error()`/`errno` after an intervening call. REQUIRED: audit the library's contract first (per-handle thread-safe → §B18 per-handle lock; global-lock-required → one process-global lock or a dedicated owning thread; init-once → `Once`/`OnceLock`); capture `errno` immediately. Grounded in CVE-2020-26235 / RUSTSEC-2020-0071/0159 (`time`/`chrono` `localtime_r`↔`getenv`/`setenv` race, shipped for years). Cross-referenced from §B18.
- **`skill/unsafe-and-ffi.md` — §B25** gains three BANNED and three REQUIRED bullets:
  - **Callback/context UAF** — BANNED: freeing/dropping a callback context before unregistration *and* in-flight-callback drain is confirmed; reclaiming `user_data` as an owning `Box<F>` in a re-invocable trampoline (drops on the first call → UAF + double-free on every later one). REQUIRED: unregister → drain (per the library's delivery model) → free, in that order; the trampoline **borrows** (`&mut *(p as *mut Ctx)`), ownership reclaimed exactly once at teardown; `// SAFETY:` states whether the library may invoke from another thread (feeds §B25a/§B18). Calibration: a wrapper that has done the drain/borrow work is not a finding.
  - **Exported entry points trusting the type system** — BANNED: an exported (`#[no_mangle]`/`#[unsafe(no_mangle)]` + `extern "C"`) fn taking `&T`/`&mut T`/`&str`/`bool`/`char`/a plain `enum` from the foreign caller (NULL/non-UTF-8/bad-discriminant → immediate UB *inside* the "safe" body, before you read it). REQUIRED: take `*const/*mut` + lengths + primitives only, validate null/align/len/UTF-8/discriminant and return an error code before minting any typed value — framed explicitly as the **read-direction mirror** of §B5's validate-before-mint (as the §B5 padding bullet is the write-direction dual).
  - **Per-function ownership audit** — REQUIRED: per wrapped C function, the `// SAFETY:` block states the ownership direction (copies/takes/returns-borrowed/returns-caller-owned) *with a citation to the specific C doc line*, because real libraries are inconsistent across the same library and between versions; a uniform wrapper is a double-free/leak for the non-matching functions; an undocumented direction is a §B5-style block-and-ask.
- **`skill/unsafe-and-ffi.md` — §B5** gains one BANNED and one REQUIRED bullet on **`union` field reads**: BANNED — reading a union field without checking the tag/discriminant that says which field the writer stored (no active-field tracking, no runtime tag → reinterpreting bits at the field's type is instant UB); a non-`#[repr(C)]` union for FFI/reinterpretation (unspecified field offsets). REQUIRED — every union read behind a tag-checked safe accessor with a `// SAFETY:` naming which write made these bits valid at this field's type; `#[repr(C)]` for FFI; and if the union only pairs with a Rust-side flag, use an `enum` (which *is* a compiler-checked tagged union). Framed as the union-shaped sibling of the existing `transmute`/`from_raw_parts` rules.
- **`skill/SKILL.md`** — §B25 added to the lettered-split parent list and §B25a to the sub-section enumeration; category→module map §B25 row → §B25 (a); §B25a added to the 🔴 Enforcement-tiers list; module-header tier line updated (§B25a 🔴). Four phrase-trigger rows (export a Rust fn to C / cdylib → §B25; call a C library from threads / is it thread-safe → §B25a; callback into Rust / user_data / trampoline → §B25; `union` / bindgen union → §B5) and five code-pattern rows (`#[no_mangle] pub extern "C" fn(&T/&str/bool/enum)` → §B25; `union` keyword / bindgen union field read → §B5; `Box::into_raw` as `user_data` + trampoline `Box::from_raw` → §B25; FFI handle across threads with no cited thread-safety level → §B25a; `last_os_error()`/`errno` after an intervening call → §B25a).
- **`commands/rust-intel-cc/fix.md`** — six routing rows (exported `extern "C"` fn segfaults on NULL/non-UTF-8 → §B25; callback UAF/double-free after owner dropped → §B25; wrong/uninitialized union field read → §B5; double-free/leak from one uniform wrapper across a mixed-ownership library → §B25; FFI wrapper races under concurrency though each handle is behind its own `Arc<Mutex>` → §B25a; `errno`/`last_os_error()` wrong after a failing call → §B25a).
- **`docs/sources.md`** — new "FFI/unsafe boundary — §B5/§B25/§B25a gap additions" subsection: six grounding entries (CVE-2020-26235 / RUSTSEC-2020-0071/0159 + Nomicon FFI; `errno` per-thread-global + POSIX contract; callback UAF via Nomicon "Targeting callbacks" + tcod-rs double-drop write-up + Reference UB; exported entry points via Reference UB list + Rust 2024 `unsafe(no_mangle)` guide; `union` reads via the Reference Unions page; per-function ownership audit via Nomicon + the "Wrapping Unsafe C Libraries in Rust" write-up). Documented-mechanism; the only numeric anchor is CVE-2020-26235's real-world shipment.
- **`skill/audit-project.workflow.js`** — `unsafe-and-ffi.md` category list gains `B25a`.
- **`README.md`** — status block for v0.4.8.
- **`package.json` + `.claude-plugin/plugin.json`** — version 0.4.7 → 0.4.8.

## [0.4.7] — 2026-07-09

**§B12/§B24 gain the crypto/secrets gaps a dedicated audit surfaced.** Six bullets, no new categories (still **58**), no tier change. Sourced from `docs/reviews/gap-audit-crypto-secrets.md` (each finding verified against the exact existing §B12/§B24 text and grounded in a citable source). Five extend §B12; one widens §B24 (kept as a bullet, not a new §B24a — it fits §B24's charter and avoids a 58→59 count churn). The words "salt", "certificate", and "aud" appeared nowhere in the skill before this release — the three highest-frequency crypto mistakes in LLM-generated web-service code. Per the v0.4.3/v0.4.6 sibling-bullet precedent (bullet-only additions, no new lettered category), this is a **PATCH** bump.
...
- **`skill/security.md` — §B12** gains five bullets:
  ...
  - **TLS validation bypass** — `danger_accept_invalid_certs(true)`, `danger_accept_invalid_hostnames(true)`, or a no-op `rustls` `ServerCertVerifier`, outside `#[cfg(test)]` → silent full MITM (CWE-295). REQUIRED: pin the internal CA via `add_root_certificate`. Calibration preserved: a `#[cfg(test)]`-scoped `danger_accept_invalid_certs` is fine.
  - **KDF salt misuse + below-floor parameters** — fixed/hardcoded/per-username salts (identical passwords → identical hashes) and degenerate Argon2/PBKDF2 work factors chosen for test speed. REQUIRED: per-credential `SaltString::generate(&mut OsRng)` and parameters at/above the OWASP floor, cited as a *living* reference (Argon2id m ≥ 19 MiB/t ≥ 2/p = 1; PBKDF2-HMAC-SHA256 ≥ 600 000 as of citation) so the numbers can be re-verified rather than silently rotting.
  - **Zeroize defeated by moves/reallocation** — extends the existing zeroize REQUIRED: the derive zeroes only the final location, and Rust moves are `memcpy`, so a returned/moved secret or a realloc'd `String`/`Vec` leaves a live copy. REQUIRED: `Box`/`secrecy::SecretBox` at creation; pre-size secret buffers.
...
- **§C2/§B24 calibration carve-out (internal-consistency fix).** §C2's "carry error context, don't collapse `?`-errors" is good general practice but the *opposite* of correct at a crypto boundary. Added a cross-referenced carve-out both ways: §C2's `#[from]` bullet now states "error context stops at the crypto boundary — see §B24", and §B24's REQUIRED bullet cites the §C2 carve-out back. A reader hitting either section first gets the reconciled advice.
- **`skill/SKILL.md`** — four phrase-trigger rows (verify a JWT / decode the token → §B12; self-signed cert / ignore TLS errors → §B12; store a password / salt / KDF params → §B12; decrypt endpoint / distinguish padding vs MAC → §B24) and six code-pattern rows (`danger_accept_invalid_certs`/no-op verifier; `Validation` without claim setters / `validate_exp = false`; literal/derived salt + low KDF params; low-level `rsa` private-key op on a request path; distinct decrypt-error variants to a client).
- **`commands/rust-intel-cc/fix.md`** — seven routing rows (cross-tenant token authenticates → §B12; TLS "fixed" with `danger_accept_invalid_certs` → §B12; identical password hashes / hashing "too slow" → §B12; secret in core dump after `ZeroizeOnDrop` → §B12; remotely timing-attackable crypto / `cargo audit` flags `rsa` → §B12; byte-at-a-time recovery from distinguishable decrypt errors → §B24).
- **`docs/sources.md`** — new "Cryptography & secrets — §B12/§B24 gap additions" subsection: six grounding entries (reqwest `danger_accept_invalid_certs` + CWE-295; `jsonwebtoken::Validation` + OWASP JWT; OWASP Password Storage floors as a living reference; `zeroize` "A note on copying data" + `secrecy` + benma 2020; RUSTSEC-2023-0071/CVE-2023-49092 Marvin; CWE-208/209 + the opaque `aead::Error`). Documented-mechanism; the only numbers are the OWASP floors, cited as re-verifiable.
- **`README.md`** — status block for v0.4.7.
