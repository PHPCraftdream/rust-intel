...
**Cycle 30 integrates the post-cycle-29 independent Sol-high follow-up; cycle 29 is superseded.** Scope: distinguish a decimal/consequent ternary expression from optional chaining at the parser boundary, and add causal controls proving that each supported and rejected spelling reaches the relevant mutation path. Cycle 29's 339-control evidence remains historical and is not silently rewritten. Verification completed: `npm run validate` passed (the main validator printed 12 skill Markdown files); direct `node dev/validate-fixtures.mjs` passed its 2 cases (the source header declares 343 sequential controls); and separate `node --check` passed for the validator and fixture runner. CI and active Codex-install parity remain pending and are not claimed. No new category, no BANNED/REQUIRED shape change - **PATCH**-shaped.
...
**The post-cycle-28 independent Sol-high review cycle 29 is integrated.** Scope: cover `case`/`default` recognition across prior-clause automatic-semicolon-insertion boundaries; add causal ternary and multi-clause controls that prove the parser reaches the relevant mutation path; and keep the control ledger physically sequential so numbering cannot silently drift. Cycle 28's 325-control evidence remains historical and is not silently rewritten. Verification completed: `npm run validate` passed (the main validator printed 12 skill Markdown files); direct `node dev/validate-fixtures.mjs` passed its 2 cases (the source header declares 339 sequential controls); and separate `node --check` passed for the validator and fixture runner. CI and active Codex-install parity remain pending and are not claimed. No new category, no BANNED/REQUIRED shape change — **PATCH**-shaped.
