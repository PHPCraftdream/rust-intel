...
- Any live value that **owns or contains a `DashMap`/`scc::HashMap` map guard**, held across `.await` — the trigger is guard **ownership/liveness**, not a particular method call or binding shape, so the guard types below are examples for your pinned major, not an exhaustive definition, and the ban applies equally to a guard held inside a wrapper — `Option<Entry>`, `Option<Ref>`, `TryResult<RefMut>`, a struct/tuple field, a `match`-arm binding — not just a bare `let`-bound value. The guard-bearing types: DashMap direct-access `dashmap::mapref::one::{Ref, RefMut}` (`Ref` shared, from a read-style `get()`; `RefMut` exclusive, from `get_mut()`); DashMap mapped `dashmap::mapref::one::{MappedRef, MappedRefMut}` (from `.map()`/`.try_map()` on an existing guard — still holding the original shard guard); DashMap iterator items `dashmap::mapref::multiple::{RefMulti, RefMutMulti}` (`map.iter().next()` — a retained item holds the shard read guard exactly like a direct `Ref`); the DashMap entry family `dashmap::mapref::entry::{Entry, OccupiedEntry, VacantEntry}` (all exclusive: `entry()` returns the `Entry` enum itself and `try_entry()` returns `Option<Entry>`, and the live `Entry` value from either producer already owns the shard write guard *before* any `.or_insert*()` call converts it to a `RefMut`); and the scc family `scc::hash_map::{Entry, OccupiedEntry, VacantEntry, ReplaceResult}` (`VacantEntry` comes from the insert-on-absent path, and `ReplaceResult` wraps whichever of `OccupiedEntry`/`VacantEntry` a replace produced — retaining or destructuring either across an `.await` is the same hazard). The DashMap guard is a synchronous per-**shard** `RwLock` — invisible (no `lock()` token), and `clippy::await_holding_lock` does **not** catch it. The shared read guards (`Ref`/`MappedRef`/`RefMulti`) coexist with each other and block only a later exclusive acquisition on the same shard; the exclusive ones (`RefMut`/`MappedRefMut`/`RefMutMulti`, the entry families) conflict exactly like a `MutexGuard`. Holding one across an init `.await` deadlocks: a parked task pins a sync lock that no longer yields the worker; every other task touching *any* key on that shard blocks synchronously, and under runtime oversubscription all workers wedge. The same hold-across-`.await` hazard applies to `scc::HashMap`'s guard, sync or `*_async`, but the lock granularity is different — current `scc::HashMap` locks per-**bucket** (each entry access is serialized by the read-write lock in the bucket containing the entry), not per-shard, and separately exposes `*_async` methods that yield instead of blocking the worker; prefer those over holding the sync guard across an `.await` in the first place, and verify the locking model against your pinned `scc` version rather than assuming DashMap's shard model applies. The producer return types differ and matter when you name them: `entry_sync`/`entry_async` return the `Entry` enum, while `get_sync`/`get_async` return `Option<OccupiedEntry>` (not the `Entry` enum), `begin_sync`/`begin_async` and `any_sync`/`any_async` return `Option<OccupiedEntry>`, and `replace_sync`/`replace_async` return `ReplaceResult` — treat the method names as examples for your pinned major, not the trigger's definition. The `*_async` methods narrow the *thread-blocking* hazard, not the *deadlock* one: `get_async` still yields an exclusive `OccupiedEntry` for that bucket, and an `Entry` from `entry_async` owns the bucket lock before any `or_insert*` conversion — holding *that* guard across a **second** `.await` — e.g. an inner call that itself touches the same bucket, directly or through a helper — can self-deadlock exactly like the sync case, just by yielding forever instead of parking the OS thread. The fix is the same either way: clone what you need out of the guard (or store `Arc<OnceCell<T>>` per §B2's own REQUIRED bullet below) and drop the guard before the next `.await`. Note §B13 endorses `entry().or_insert_with()` for *synchronous* check-then-act atomicity only — that endorsement is void the instant an `.await` follows the guard.
...
**The trap**: dropping a pending future ends polling of that future; whether it stops an underlying operation depends on the API and ownership (§B8/§B21). An `.await` is a potential suspension point, not a guaranteed yield: a ready child can complete within the same poll, and cancellation cannot preempt an ongoing synchronous poll. Cancel safety is **not visible in any signature**. Borrow checker doesn't help. Clippy doesn't help. Documentation for each tokio function must be read individually (`AsyncReadExt::read` is cancel-safe, `read_exact` is not). In the 2026 field report, **zero** models across the timeout-using benchmark tasks spontaneously mentioned cancel safety; when asked directly, they answered "yes, it's cancel-safe" confidently and incorrectly in ~50% of cases.
...
async fn read_message(reader: &mut OwnedReadHalf, buf: &mut Vec<u8>) -> io::Result<Bytes> {
    loop {
        ...
        let n = reader.read(&mut chunk).await?;   // cancel-safe
        if n == 0 {
...
**The trap**: an `async fn` body's required work is abandoned when its future never reaches a polling consumer. The code can compile, and a test that checks only that the caller returned can stay green while the intended request, write or update inside that body never runs. The defect is the missing execution path for the future, not merely a missing `.await` token at its construction site.

**Why this happens**: LLMs confuse constructing an operation with driving it to completion. An `async fn` / `async` body's work starts when polled, but this is not a blanket promise about every `Future`-returning API: an ordinary `fn -> impl Future` executes its synchronous prefix at the call, and argument evaluation and destruction can have effects too. A `JoinHandle` observes already-scheduled work (§B21). Inspect the actual API and the future's eventual owner.
...
**BANNED**:
- `let _ = some_async_fn(...);` dropping a lazy future whose body was required to run, without handing it to a polling owner.
- Calling an async function and abandoning its required work, with no caller, combinator or executor responsible for driving the result. Deliberate cancellation/discard under the operation's contract is not a forgotten-await finding.
- `let _fut = async_fn();` whose future is never driven or forwarded and drops with required work unperformed. Boxing it (`Pin<Box<dyn Future>>`), adding an adapter (`.map(...)`, `.then(...)`), or obtaining it from an `async ||` closure does not itself schedule that lazy body.
- An `impl Future` value that reaches no polling owner despite an execution obligation. A local binding inside a non-`async` factory is not itself a defect: returning or forwarding that future to a caller that drives it is valid. Trace the ownership/consumer chain instead of inserting an eager spawn merely because the factory contains no `.await`.
...
**REQUIRED**:
- Every required asynchronous operation reaches a polling owner: direct `.await`, a driven combinator/collection, a caller receiving the future, or the executor-appropriate spawn primitive. Storing a lazy future alone does not drive it; a `JoinHandle` instead represents work already scheduled by a spawn. Verify the intended completion/observation path.
- For fire-and-forget **lazy work**, use the executor-appropriate spawn primitive: `tokio::spawn` for a `Send` future, or `tokio::task::spawn_local`/`LocalSet::spawn_local` for a `!Send` future confined to a `LocalSet`. If the API already scheduled the operation, follow its existing handle/observation contract instead of adding another spawn. The §B21 supervision rules (hold vs. detach, graceful shutdown) apply to the resulting handle.
- Enable `#[warn(unused_must_use)]` at crate level. Verify the `#[must_use]` warning fires for ignored futures in clippy output.
- For functions returning `impl Future`, follow the result to its eventual consumer; flag required work that is actually abandoned, not the absence of `.await` at the factory site.
...
**REQUIRED**:
- For a bounded synchronous/blocking section that would otherwise monopolize a worker (a large compression/hash/parse operation or a blocking library call), use `tokio::task::spawn_blocking(|| { ... }).await?` or an appropriate CPU executor. This moves the section off the async worker; admission and work-size limits still apply (§B7/§B14). Short cooperative chunks follow the scheduling guidance below.
...
- `tokio::task::yield_now().await` offers cooperative scheduling, not offloading or preemption. It can allow other runnable tasks to progress between bounded chunks, including deliberately cooperative CPU work, but promises neither a particular next task nor an I/O-driver poll before resumption; enclosing combinators may prevent the yield from reaching the executor at all. It cannot make a long synchronous section or blocking syscall interruptible. Offload those sections; for short chunks, measure the scheduling/latency trade-off rather than classifying cooperation solely by an IO-bound/CPU-bound label.
...
**Forgotten Waker in manual `Future::poll`**: when implementing `Future` by hand, returning `Poll::Pending` without first registering the current task's `Waker` causes the task to hang forever — nothing will ever wake it. The executor doesn't poll spontaneously. And registration must be *ordered* against completion, not just eventual: a completion that lands between the poll's observation of "not ready" and the waker being stored sees no registered waker and wakes nobody, so the task sleeps forever with its completion already delivered.

- Before any `return Poll::Pending`, ensure a re-poll is arranged one way or another — and when *this* future owns the external completion source (required whenever nothing else will wake it), the registration must provably close the check/complete/register race, using one of these protocols (each orders registration against completion):
  - **register-then-check**: store `cx.waker().clone()` into the shared waker slot FIRST, then read the readiness state; if it is already ready, return `Poll::Ready` now (the stored waker is never called or is called redundantly — a spurious wake is legal and costs one extra poll).
  - **check/register/recheck**: read the state, store the waker, then re-read the state; if the recheck sees ready, return `Poll::Ready` instead of `Pending`.
  - **a lock/atomic protocol with the same ordering proof**: completion publishes readiness (e.g. a `Release` store of the flag, or a state write under the same mutex) *before* it takes or reads the waker slot, and poll registers the waker *before* its final `Acquire` read of the flag — then a completion racing the registration either finds the waker already stored or is observed by the recheck. `futures::task::AtomicWaker` implements exactly this register-vs-wake protocol and is the default choice over a hand-rolled slot.
  The naive shape — observe "not ready", store the waker, return `Pending`, with nothing re-read after the store — is the banned completion-between-check-and-register shape even though a waker was "eventually" stored.
- When the implementation delegates to a child future (`self.inner.poll(cx)`) or otherwise self-schedules before returning `Pending`, confirm the delegated call itself registers the waker and don't add a redundant clone of your own.
- Default to combinators (`async/.await`, `FutureExt`, `tokio_util::sync::PollSender`) rather than manual `Future` impls.
- If hand-rolling is unavoidable, write a comment naming who will call the stored waker, under what condition, and which of the protocols above closes the race.
...
- For running blocking/CPU-bound work from inside async, use `tokio::task::spawn_blocking` (separate blocking-thread pool) or `tokio::task::block_in_place` (runs blocking code on the current worker without starving sibling **tasks** — note this is for async-calls-blocking-code, *not* a sync-to-async bridge; you still cannot `.await` inside it without a `Handle`). **Same-task qualification:** a sibling *branch* of the current `join!`/`try_join!` is not a sibling task — all branches of a `join!` are one future polled as one task, and while this branch is inside `block_in_place` nothing polls the other branch, on any runtime flavor. If the blocking call waits on that sibling branch's output (a value it sends over a channel, sets in shared state, or otherwise produces later), the wait can never complete: the sibling needs the thread this branch is holding — deadlock. For that dependency shape use `spawn_blocking` (the waiting branch parks at a real `.await`, so `join!` keeps polling its siblings) or genuine task separation (`tokio::spawn` the dependency as its own task); `block_in_place` inside a `join!` branch is sound only when the blocking call's completion does not depend on the sibling branches (inputs already in hand, or produced by independently spawned tasks). **`block_in_place` panics on a current-thread runtime — it requires the multi-threaded runtime** (it works by handing the worker's other tasks to a sibling thread, of which a current-thread runtime has none). Since `#[tokio::main(flavor = "current_thread")]` and `#[tokio::test]` both default to current-thread, this panic is easy to hit; use `spawn_blocking` (works on both flavors) when the runtime flavor is not guaranteed multi-threaded. The one documented exception to "never nest `block_on`" is `block_in_place(|| Handle::current().block_on(fut))` — tokio's own docs (docs.rs/tokio/latest/tokio/task/fn.block_in_place.html) show this as the supported way to reenter the async context from inside `block_in_place`, and it is legitimate **only** on a multi-threaded runtime (the same restriction `block_in_place` itself already has, per the panic note above). Outside that narrow shape — any nested `block_on` on a current-thread runtime, or a nested `block_on` not wrapped in `block_in_place` — remains banned.
...
**BANNED**:
- `notify.notified().await` in the **concurrent-waiters + condition-check** shape — wakeups can race with `notify_one()` and be lost: two concurrent waiters each check the condition, both `notify_one()` calls fire but only a single permit is stored, one waiter consumes it and proceeds, and the other sleeps forever with its predicate already true. Simply creating + `pin!`-ing a `Notified` future does **not** arm it for wakeups; only `.enable()` (or the first poll) adds it to the notify list. And arming alone is not the whole contract: the single-`if` shape resumes after the notification **without rechecking the predicate** — with competing consumers, another waiter can consume the state between this waiter's wakeup and its resumption, so it proceeds on a predicate that is already false. The canonical pattern is enable-before-check **inside a condition loop** that replaces the spent future after every wakeup:
  ```rust
  let notified = notify.notified();
  tokio::pin!(notified);
  loop {
      notified.as_mut().enable();        // (re)arm BEFORE the check — closes the check/enable race
      if condition() {                   // predicate re-checked on EVERY iteration
          break;
      }
      notified.as_mut().await;           // consume one wakeup
      notified.set(notify.notified());   // spent future replaced — never re-polled
  }
  ```
  Both halves are load-bearing: `enable()` is what actually arms the wakeup before you inspect the condition (a `notify_one()` landing between the check and the await is not lost), and the loop + `set()` re-check the predicate after every wakeup because a competing consumer may have consumed the state before this waiter resumed. Never re-`.await` a `Notified` future that already completed — it is spent and the loop would wait on it forever; replace it with `set(notify.notified())` (this is tokio's own multi-consumer example shape: enable → check → await → set, docs.rs/tokio/latest/tokio/sync/struct.Notify.html). Negative control for the loop: on a one-item state, a second waiter woken after the first consumed the item must re-check and go back to sleep — the single-`if` shape lets it resume and act with the state already gone.
  Scope the ceremony to the races it closes — plain single-consumer usage needs none of it. With at most one waiter, `notified().await` is correct as-is: a `notify_one()` that lands before the await stores a permit in the `Notify`, and the next `notified().await` consumes it, so no wakeup can be lost (tokio's own single-consumer channel example is exactly check-then-`notified().await` in a loop, no `enable()` — the docs state this lost-wakeup failure "can only happen if there are two concurrent calls to `recv`"). Shapes that require the ceremony (or equivalent care): multiple concurrent waiters competing for the single permit — the loop above, since any of them can lose the state to the others — and `notify_waiters()`, which stores no permit at all — it wakes only the `Notified` futures already created at the instant it fires, so a `notify_waiters()` before any waiter exists wakes nobody.
...
§B8 covers required lazy work that never reaches a polling owner; this category covers already-scheduled work whose spawning code loses its handle-based observation or control.

**The trap**: `tokio::task::JoinHandle::drop()` **does not abort the task**. Still-active work remains scheduled and may retain resources beyond the spawning scope, while unobserved results are discarded. A short task can hide this in tests; a long-lived task can retain connections, locks, file descriptors or CPU for much longer than intended. `std::thread::JoinHandle` also detaches on drop. Decide the lifetime and result-observation policy rather than treating handle destruction as task termination.

**BANNED**:
- Dropping a `tokio::task::JoinHandle` without `.await`, `.abort()`, or an explicit "detached on purpose" comment. Default drop detaches still-active work and loses that handle's result/control; an independently retained `AbortHandle` or cancellation signal may still control the task but does not recover the dropped join result.
- Storing `JoinHandle`s in a `Vec` that is later dropped without the intended completion policy — this detaches any still-active tasks and discards unobserved outputs; resources can remain held until those tasks finish.
...
- Assuming a detached task's panic is returned to its spawner. With unwinding panics, Tokio catches it and exposes a panic `JoinError` through the join handle; dropping that handle loses this result, although the default panic hook still prints to stderr. Under `panic = "abort"`, the process terminates instead (§B4). If failure must trigger alert/restart/shutdown logic, hold and await the handle (or use a `JoinSet`), or provide an explicit reporting path.
...
- `.abort()` requests forced cancellation; it is not graceful shutdown or immediate preemption. The runtime can drop the task's future after regaining control, not between arbitrary ready `.await`s in one ongoing poll. Synchronous destructors run on that drop, but the abandoned future cannot finish its async cleanup. Where graceful cleanup is required, use abort as the fallback after cooperative shutdown exceeds its grace period: wait with `tokio::time::timeout(grace_period, &mut handle).await`; on expiry, request abort and await the still-unjoined handle to observe completion/cleanup. Explicitly disposable work may use immediate forced cancellation. Handle all outcomes: normal completion may win the race, cancellation yields a cancelled `JoinError`, and an unwinding panic yields a panic `JoinError`. Do not assume every post-abort join is cancelled, or await the handle again if the grace-period wait already consumed its result. A running `spawn_blocking` closure cannot be aborted (§B11); it needs cooperative cancellation.
...
- Document the type's actual fallback: what synchronous `Drop` releases and which guarantees require `.close().await` (flush, acknowledgement, protocol close, pool readiness). Missing async close is not automatically an OS-handle leak when the owned handle still closes on drop. Specify the disposition after an error or cancellation of the consuming close future as well (§B4/§F3).
- Where synchronous best-effort release is available, use it as the `Drop` fallback and distinguish it from the explicit async close guarantee. Report a resource leak only when the type's actual ownership/cleanup behavior leaves that resource unreleased.

## §B23. `select!` arm side effects under cancellation

This category applies §B3 to `select!`: side effects must survive any actual cancellation boundary or remain outside it. Selection drops the losing futures it owns; a losing branch that only borrows an externally owned future leaves that future available to its owner. Identify which value is actually dropped before claiming cancellation or lost progress.

**The trap**: `tokio::select!` polls branch futures on one task and runs the handler for a selected ready output whose pattern matches. Dropping an owned losing future can abandon partial work (DB write, file flush, channel send); borrowing a persistent future can preserve that progress, as in the reservation example below. The compiler does not prove the operation's cancellation contract, and tests with only one ready branch miss the losing paths.

**Disabled branches still construct their futures.** A false `, if condition` prevents polling, not evaluation of that branch's future expression. A synchronous factory prefix or an argument expression can therefore perform side effects even for a disabled branch (§B8). Guard construction outside `select!`, or defer the work into an `async` body when polling is the intended gate; an enabled body still needs the cancellation discipline below.
...
- Pattern: `select! { _ = ch.send(x) => ... }` is **not** cancel-safe even on `tokio::sync::mpsc::Sender::send`. Per tokio's documentation: if `send` is cancelled in a `select!` arm, the message is **dropped and lost**. The future's resolution distinguishes "sent" from "cancelled-and-lost", but the data is gone either way. For cancel-safe channel send inside `select!`, use the two-step pattern: `let permit = ch.reserve().await?;` (cancel-safe — only acquires capacity, transmits nothing), then `permit.send(x)` (synchronous, infallible at that point). Other channel libraries (`flume`, `async_channel`, custom) require their own per-API verification. `reserve()` protects **one** of two real cancellation properties and sacrifices the other: it preserves the **owned message** but not the sender's **queue position** — dropping the pending `reserve()` future (what a lost `select!` arm does) abandons the sender's place in the waiters queue, and re-creating it on the next iteration re-enters at the back, so under sustained contention a `loop { select! { ... ch.reserve() ... } }` can lose the race for every freed slot and starve that sender indefinitely — repeated cancellation starves the operation even though not one message was lost. When queue position/fairness matters, preserve the pending future outside repeated selection instead of re-entering with a fresh one each iteration:
  ```rust
  let reserve = ch.reserve();      // created ONCE — keeps its queue position
  tokio::pin!(reserve);
  let permit = loop {
      tokio::select! {
          p = reserve.as_mut() => break p?,        // same future re-polled every iteration
          _ = shutdown.notified() => return Err(...),
      }
  };
  permit.send(x);                  // synchronous, infallible at this point
  ```
  The same two-property distinction applies to fair lock/semaphore acquisition: tokio's `Semaphore` is documented fair ("permits are given out in the order they were requested"), and its `acquire` cancel-safety note says verbatim "cancelling a call to `acquire` makes you lose your place in the queue" (the same wording as `Notify::notified`) — so a cancelled `lock()`/`acquire()` future loses no value *and* its queue position; re-creating the acquisition future on every loop iteration can starve it the same way. Value preservation and queue position are two different, both-real cancellation contracts — state which one a design needs, and hold the future across iterations when position matters.
- Side-effecting `async` helpers called from `select!` arms without a documented cancel-safety annotation per §B3.

**REQUIRED**:
- For every future that selection or its enclosing scope can actually drop while pending, prove cancellation safety at its possible suspension points: idempotent effects, proven rollback, commit-observable progress, or resumable state without duplication under §B3's drop-and-recreate test. Atomic race-freedom alone is insufficient. If a borrowed future survives selection, verify that its owner keeps driving it and honors its eventual completion/cancellation contract.
- Move side effects **after** the `select!` returns, on the winning branch only:
  ...
  ```
- If an operation must continue after selection moves on, retain and drive its future through an owner whose lifetime covers the operation, or spawn supervised work and keep its `JoinHandle` (§B21). Merely keeping a future pinned outside `select!` does not make it run in the background or survive the drop of that owner (§B8).
...
- For `wasm32` targets, scope every claim to the actual target triple and enabled features — never write or assume a blanket "WASM is unsupported": tokio documents limited WASM support whose usable feature set is target- and feature-dependent (per the tokio crate docs' `WASM support` section for your pinned version — without `tokio_unstable`, the `sync`/`macros`/`io-util`/`rt`/`time` features are supported and enabling any other feature, `full` included, fails to compile; with `tokio_unstable`, `tokio::net` is additionally available on `wasm32-wasip1`/`wasm32-wasi`, with socket-creation caveats). The browser-target restrictions below describe `wasm32-unknown-unknown` and browser/wasm-bindgen environments — they are not a ban on WASI: on `wasm32-wasip1`/`wasip2` tokio's supported feature subset works, and `tokio::time::sleep` is usable there because WASI provides timers. Concretely, on `wasm32-unknown-unknown`: no threads, no blocking I/O, no `tokio::time::sleep` (use `gloo-timers` or equivalent); no `std::time::Instant::now()`/`SystemTime::now()` — both **panic** there (§B27); use `web_time::{Instant, SystemTime}` (re-exports `std::time` on non-wasm targets). `wasm32-wasip1`/`wasip2` are unaffected by that panic. Check the target and the enabled feature set before writing WASM-targeted async code; treat "works on WASI" and "works in a browser" as different compatibility statements.
...
- Recording secret material through `#[tracing::instrument]`: automatic argument fields, explicit `fields(...)`, and `ret`/`err` events are **independent paths**. `skip` / `skip_all` suppress automatic arguments only — `#[instrument(skip_all, ret)]` still records the returned token, and `#[instrument(skip_all, err)]` still records a secret-bearing error. For arguments, use `skip(password)` only when the remaining arguments are approved to log, or `skip_all` with approved metadata such as `fields(operation = "login")`; an email is not automatically non-sensitive metadata. For outputs, omit `ret`/`err` or redact the formatter actually selected: `ret` defaults to `Debug` (supports `ret(Display)`) and records only the `Ok` value for a `Result`; `err` records `Err` via `Display` by default (supports `err(Debug)`). Redacting only `Debug` does not protect a `Display` event. Explicit `fields(...)` must not reintroduce skipped secrets or disallowed PII. Assert on captured spans **and** events for the relevant argument, success and error paths. 🔴 for secret leakage on any of these paths (§B12); 🟡 for PII/payload-volume policy (§E5).
...
**REQUIRED**:
- Wrap spawned futures with the parent span: `tokio::spawn(my_fut.in_current_span())` (requires `use tracing::Instrument;` in scope). The `in_current_span()` adapter binds the *current* span to the future at spawn time, so it is restored when the future is polled. The explicit form `tokio::spawn(my_fut.instrument(parent_span))` is the equally valid sibling method on the same `Instrument` trait — use it whenever the parent span is *not* `Span::current()` at the spawn site (captured before an earlier `.await`, stored in a struct, passed in from a caller); do not contort code to make `in_current_span()` apply, and do not flag `.instrument(parent)` as a violation — the ban is on spawning with *no* span attached.
- For `tokio::task::spawn_blocking`, capture the span explicitly and re-enter it inside the closure:
  ...
  ```
  Note this restores only the *span*. `tokio::task_local!` values do **not** travel into a `spawn_blocking` closure (it runs on a fresh blocking-pool thread with no task scope), so capture every `task_local`/request-context value (tenant, auth, request-id, locale) *before* the call and move it into the closure explicitly. The failure there is *loud*, not a silent stale read: accessing a tokio task-local outside its scope panics (`with`/`get`) or returns `Err(AccessError)` (`try_with`/`try_get` — tokio's `LocalKey` docs) — it never returns the previous request's value. The silent cross-request corruption is the *std* `thread_local!` hazard above (values persist on the pooled thread); the task-local hazard is a guaranteed panic/error mid-request — still a defect (the request dies at the access site), just a different one.
