...
- Surface every spawn call whose returned `JoinHandle` is dropped (not held, not awaited, not detached-by-design) in the post-flight summary — this applies to every Tokio API that hands back a `JoinHandle`, not just `tokio::spawn`: `tokio::task::spawn_local` and `LocalSet::spawn_local` detach on drop exactly the same way, and a dropped handle from either is the same silent leak.
...
**BANNED**:
- `tokio::select!` arm that performs an **unrecoverable, non-idempotent** side effect inside the arm's pending future (between the first `.await` and the future resolving) — at cancellation, the side effect is either half-done or not done, and there is no recovery hook. Not every side effect in that position qualifies: an atomic operation, an idempotent write (safe to redo with the same result), or a transactional one that can be rolled back or safely retried is cancel-safe in that position by construction — the finding is unrecoverable partial progress specifically, not "a side effect exists here at all".
