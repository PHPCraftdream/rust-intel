...
A genuine borrow-checker rejection (`compile_fail` shape), not an "empty intersection" hand-wave: a cache of references cannot outlive any source it has borrowed from. Read that rejection correctly: rustc refusing to compile is the **intended API constraint being enforced**, not a silent dangling-reference bug that already shipped — the witness proves the design refuses to let the cache outlive a source, which is exactly what a lifetime witness (compile-fail use-case/design test) is for. And a cache of borrowed input is itself **sound** whenever it is deliberately bounded by that input's own lifetime: `HashMap<String, &'a str>` living and dying with `'a` — one call's locals, or an arena whose `'arena` is part of the type's public contract — is a legitimate, common pattern the borrow checker enforces mechanically. The defect this category targets is only the shape where one `'a` couples the cache to *each call's* input while the API presents the cache as long-lived.

**Prompt triggers that produce this**: "add caching to this function", "memoize", "speed up by storing results", "build a lookup". Whenever the user mentions an input-derived borrow captured into a cache/container that outlives the call — whether returned or written through an argument — this category activates.
...
- Calibration: **owning the cache is the common default because it removes the coupling, not a Rust safety requirement.** A borrowed cache (`HashMap<String, &'a str>`) is legitimate whenever `'a` genuinely bounds the cache and that bound is visible in the API; pick owned vs borrowed per use case.
...
**The trap**: exposing `'a` in a *public* function signature when the lifetime is an implementation detail. The function compiles, the lifetime is genuine, and the signature is technically more "zero-copy" than the alternative — but callers who *store or pass along* the result must now track that lifetime in their own types and signatures (plain call sites stay annotation-free: elision writes them, so a borrowed return does not force every caller to write explicit lifetime annotations).
...
**A tradeoff, not a safety rule — borrowed public returns are an idiomatic Rust "view" pattern**:
```rust
// Callers do NOT write explicit lifetime annotations here; elision handles the common
// call sites (`let doc = parse(s);` compiles with no lifetimes written):
pub fn parse<'a>(source: &'a str) -> Document<'a> { ... }
```
Returning a borrow is not inherently a design smell. What borrowing costs is *flexibility*, not annotation burden: a borrowed result cannot be stored past its source without cloning, so callers who must keep the data clone it or fetch an owned variant. Whether a published API returns owned or borrowed by default is an API-design preference (the API guidelines' C-CALLER-CONTROL "caller control" axis), not a universal Rust safety rule — but in a published library it is a visible commitment either way, so make it deliberately:

**REQUIRED**:
- Owned return types (`pub fn parse(source: &str) -> Document { ... }` where `Document` owns its data) are the common ergonomic default — label that default a **tradeoff** (freely storable results at the cost of copying), not a universal Rust safety rule; a borrowed view is idiomatic wherever callers use the result immediately.
- If you return a borrowed view, document that the result borrows the input, and consider exposing both variants (`parse` returning owned + `parse_borrowed` returning the lifetime-parameterized version) so callers choose per use case.
...
**The trap**: *changing* a blanket `impl<T: Display> Bar for T` once it has been published is a versioning landmine — adding a new blanket impl (consumers' `impl Bar for MyType` becomes a coherence conflict, `E0119`, two impls now overlap) or narrowing one (a downstream type that relied on the wider impl now fails an unsatisfied-bound error instead). The breakage surfaces months later on consumer CI, not the author's. Deciding **at first release** to ship an unsealed extension trait with a blanket impl is a different thing entirely: an initial API design decision, not a break — no downstream code has depended on the coverage yet, so there is nothing to break. It is still a permanent commitment to the trait's coverage; make it deliberately and document it (see REQUIRED). Being unsealed is not, by itself, wrong.

**REQUIRED in any `pub` API**:
- A blanket impl at **initial release** is a legitimate design decision, sealed or not: the common unsealed extension-trait shape (`impl<T: Display> MyExt for T`) ships fine — it removes no coverage anyone has relied on yet — provided you accept that its coverage is part of the contract from day one and document that commitment. The 🔴 flag on "blanket impl in pub API of a published library" marks that commitment for a conscious decision; it does not mean the pattern is wrong.
- Seal when you want freedom to add or remove impls without `E0119`-overlap conflicts with downstream impls — blanket `impl<T: Bound>` only when the trait is **sealed** (private supertrait the crate controls):
  ```rust
...
- Once **published**, changing coverage is the actual semver hazard (see the trap above): adding a new blanket impl or tightening an existing one's bounds is a **major** breaking change. An unsealed published blanket gives you no freedom to make such a change safely; sealing is what buys partial freedom (no downstream-impl conflicts — narrowed coverage can still break downstream callers, so treat any tightening as breaking regardless). When growth is plausible and you did not seal, write per-type impls or use a marker trait the crate exposes for opt-in, so future coverage changes stay additive.
- For any public trait being added, explicitly state in a comment whether it is sealed or open to external impl.
- Respect the actual orphan/coherence rule (the Reference's *trait implementation coherence* section; RFC 2451 re-rebalancing) — not the simplified "foreign trait + foreign type is always banned" framing. For `impl<P1..=Pn> ForeignTrait<T1..=Tn> for T0`, the impl is legal when **at least one of `T0..=Tn` is a local type**, and no *uncovered* type parameter (one not appearing in the bound of another type parameter) occurs in `T0..Ti-1`, i.e. before the **first** local type `Ti`. Consequences the simplified rule gets wrong: `impl From<Local> for Vec<Local>` is **legal** — the local type appears among the trait's parameters, so coherence is satisfied with no wrapper — and so is `impl From<Local> for ForeignType` (first local type sits in the trait args; nothing uncovered precedes it). What is actually banned: **no local type anywhere** (`impl From<String> for ForeignType`), or an uncovered type parameter **before** the first local type (`impl<T> ForeignTrait<T> for ForeignType`). Check the real shape before reaching for a wrapper — many "orphan rule" hits are legal under the full rule.
- When a wrapper genuinely is required (the truly-banned shapes above), newtype it: `pub struct MyWrapper(Foreign);` with the inner tuple field **private by default** — trait coherence has no requirement that the field be `pub`; expose accessor methods, or make the field `pub` only for a specific reason.
- An ordinary newtype wrapper used purely for type safety/API ergonomics is already zero-cost at the type level — constructed and destructed at compile time, no runtime overhead — and needs **no** `#[repr(transparent)]` for that. `#[repr(transparent)]` is an intentional **layout/ABI commitment**: it guarantees the wrapper's layout, size, and alignment are identical to `Foreign`, making it ABI-compatible for documented FFI/transmute-adjacent use cases — require it only when that specific representation contract is actually wanted, and note the commitment in a comment when you do. It does **not** by itself make `transmute` sound: the §B5 value-validity, ownership, provenance, and lifetime proofs still apply. Without `#[repr(transparent)]`, the layout is `#[repr(Rust)]` (stable attribute, unspecified layout) — a limitation only when code relies on layout/ABI, not for ordinary type-level use.
...
- Mark a public `struct` `#[non_exhaustive]` when future fields are plausible. This forbids downstream struct-literal construction (`Foo { a, b }`), so the type needs a deliberate **construction story** — the constructor/builder is the common one but not the only one. Ship whichever fits: a constructor (`Foo::new(...)`) or builder; an `impl Default` (consumers call `Foo::default()`); `From`/`Into` conversions implemented in your crate; or dedicated factory functions. Or decide the type is intentionally **producer-only** — your crate creates every instance, consumers only read the public fields — with no public construction path at all, a valid story for result/report/error-like types. What is not OK is leaving the type *accidentally* unconstructable: consumers who are supposed to build it and cannot.
...
- It is **not** retroactive insurance: adding `#[non_exhaustive]` to an *already-published, genuinely exhaustive* type is itself a breaking change — a type whose fields are all currently public (so downstream can still construct it with a struct literal and match it exhaustively today), or an enum/tuple struct with no other factor already blocking that. This does **not** extend to the calibration exception right above: a named-field struct that *already* has at least one private field is not exhaustive in the practical sense — downstream already cannot construct or exhaustively match it — so adding `#[non_exhaustive]` there is not a *new* break; it is still worth deciding deliberately (it does change intent/documentation), but it does not carry the same breaking-change weight as adding it to a genuinely exhaustive type. Decide at first publication for the genuinely-exhaustive case, flag the decision inline (at write time).
...
- For library crates, keep two distinct questions apart — rustc itself computes them separately, and conflating them is what makes `unreachable_pub` look like it should catch a case it structurally cannot. **Reachable** (rustc's own term, used in its diagnostics): an external crate can obtain a value of the type or a call into the item at all — via direct access, a `pub use` re-export, *or* signature leakage (a public function returning or taking it, letting an external caller obtain a value through inference even with no import path to name). **Nameable**: an external crate can write an import path to the item — which requires an unbroken chain of `pub` modules, `pub use` re-exports, **or a public type alias along the path** (`pub type PublicS = hidden::S;` already names the type downstream as `the_crate::PublicS` — a `pub type` alias is a third naming path alongside the other two, matching the central trigger row) — and is strictly narrower than reachable. (A fourth naming route sits outside those three: a public **associated-type projection** — when a public trait's impl fixes `type Output = Hidden;`, downstream code can name the hidden type as `<Factory as Has>::Output` with no module path, re-export, or free alias anywhere; established by a two-crate metadata probe on Rust 1.97 — so public module paths, re-exports, and free aliases are **not** the only routes by which a hidden type becomes nameable.) An item that is neither is genuinely internal and not a semver commitment; every `pub fn`/type that is nameable is a semver commitment, full stop. The gap in between — **reachable but not nameable** — is the trap: a `pub` item behind a private module ancestor with no `pub use`/`pub type` path is not nameable, but if a *public function's signature* returns or takes it, it becomes reachable anyway (an external caller writes `let x = my_pub_fn();` and calls the type's public methods with no import path ever named), so it is functionally part of the API surface without being directly importable. Enable rustc's `unreachable_pub` lint (allow-by-default; catches an item that is genuinely *not reachable* — the accidental-`pub` case, not the leaked case) **and** `unnameable_types` (also allow-by-default; catches the reachable-but-not-nameable case above) — `unreachable_pub` alone does not fire on a signature-leaked type precisely because rustc's own visibility computation already counts it as reachable, which is exactly why the second lint exists. One known gap: `unnameable_types` does not yet model the alias naming path — on an alias-exposed type like `PublicS` it still reports the type as reachable-but-not-nameable even though the alias makes it nameable (rust-lang/rust#120146, open) — so a lint hit there is a known false positive, not evidence the type is outside the public surface; silence it with a documented `#[allow(unnameable_types)]` or replace the trivial alias with `pub use`. More broadly, treat `unnameable_types` as a **triage signal — useful, cheap, but not an exact nameability oracle**: its model does not account for every naming route (the public associated-type projection above is one), so a clean run does not prove a type is unnameable and a hit does not by itself settle whether the type is part of the public surface; treat lint output as leads to check by hand. Flag a newly-`pub` item inline (at write time) so the user can confirm the visibility decision.
