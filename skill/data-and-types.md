...
- Be aware JSON object **duplicate keys are last-wins** by default (RFC 8259 §4 says names "SHOULD be unique" but implementations differ): `{"amount":1,"amount":1000000}` deserializes to `1000000` with no error, and the differential — an upstream proxy/WAF reading the *first* value, the Rust service the *last* — is a parser-differential request-smuggling vector. Reject or canonicalize duplicate keys at a security boundary rather than trusting the default.
