...
| "parse CSV/JSON/URL by hand", "write my own parser for", "quick script, no dependencies", "avoid adding a crate for this", "just use split/format!", "compare version strings", "is this version newer", "match a glob/wildcard pattern" | §C12/§C12a reinventing a solved problem | a naive `split`/`format!`/regex shape passes an easy test and is silently wrong on a real input — propose the catalog's named crate (per §A1) before hand-rolling |
