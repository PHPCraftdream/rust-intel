...
| "union", "bindgen union", "tagged union from C", "sockaddr/VARIANT/event struct" | §B5 union field reads | reading a union field without checking the tag is instant UB; `#[repr(C)]` + tag-checked accessor + `// SAFETY:`; a union that only pairs with a Rust flag should be an `enum` |
